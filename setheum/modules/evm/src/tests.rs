...
use super::*;
use mock::{evm_module, RuntimeCall, RuntimeEvent, *};
...
fn should_calculate_contract_address() {
	new_test_ext().execute_with(|| {
		...
		let metadata = StackSubstateMetadata::new(1000, 1000, &SETHEUM_CONFIG);
		let state = SubstrateStackState::<Runtime>::new(&vicinity, metadata);
		let mut executor = StackExecutor::new_with_precompiles(state, &SETHEUM_CONFIG, &());
...
// send 1 eth via transfer
		...
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Stopped));
		assert_eq!(balance(charlie()), 3 * amount + dollar_see);
		assert_eq!(
			eth_balance(charlie()),
			U256::from(convert_decimals_to_evm(balance(charlie())))
		);
		assert_eq!(balance(alice()), alice_balance - 3 * amount - dollar_see);
		assert_eq!(
			eth_balance(alice()),
			U256::from(convert_decimals_to_evm(balance(alice())))
		);
...
// call method `set(123)`
					RuntimeCall::EVM(evm_module::Call::strict_call {
