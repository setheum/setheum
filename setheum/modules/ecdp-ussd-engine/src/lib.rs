...
use module_support::{
	AddressMapping, EcdpUssdTreasury, EcdpUssdTreasuryExtended, SwapManager, EVMBridge, EcdpEmergencyShutdown, ExchangeRate,
...
pub const OFFCHAIN_WORKER_DATA: &[u8] = b"setheum/ecdp-ussd-engine/data/";
pub const OFFCHAIN_WORKER_LOCK: &[u8] = b"setheum/ecdp-ussd-engine/lock/";
pub const OFFCHAIN_WORKER_MAX_ITERATIONS: &[u8] = b"setheum/ecdp-ussd-engine/max-iterations/";
...
/// cap, ECDP's owner cannot issue more stablecoin under the collateral
/// type.
	pub maximum_total_debit_value: Balance,

/// Liquidation ratio, when the collateral ratio of
/// ECDP under this collateral type is below the liquidation ratio, this
/// ECDP is unsafe and can be liquidated. `None` value means not set
	pub liquidation_ratio: Option<Ratio>,

/// Liquidation penalty rate, when liquidation occurs,
/// ECDP will be deducted an additional penalty base on the product of
...
/// of ECDP so that the current collateral ratio is lower than the
...
/// The default liquidation ratio for all collateral types of ECDP
		#[pallet::constant]
...
/// The default liquidation penalty rate when liquidate unsafe ECDP
		#[pallet::constant]
...
/// The ECDP USSD treasury to maintain bad debts and surplus generated by CDPs
		type EcdpUssdTreasury: EcdpUssdTreasuryExtended<Self::AccountId, Balance = Balance, CurrencyId = CurrencyId>;

/// The price source of all types of currencies related to ECDP
...
/// Emergency shutdown.
		type EcdpEmergencyShutdown: EcdpEmergencyShutdown;
...
/// Evm Origin account when settle ERC20 type ECDP
...
/// The ECDP must be unsafe status
		MustBeUnsafe,
/// The ECDP must be safe status
		MustBeSafe,
/// Invalid collateral type
		InvalidCollateralType,
/// Remain debit value in ECDP below the dust amount
		RemainDebitValueTooSmall,
/// Remain collateral value in ECDP below the dust amount.
/// Withdraw all collateral or leave more than the minimum.
		CollateralAmountBelowMinimum,
/// Feed price is invalid
		InvalidFeedPrice,
/// No debit value in ECDP so that it cannot be settled
		NoDebitValue,
/// System has already been shutdown
		AlreadyShutdown,
/// Must after system shutdown
		MustAfterShutdown,
/// Collateral in ECDP is not enough
		CollateralNotEnough,
...
/// Settle the ECDP that has debit.
		SettleCDPInDebit {
			collateral_type: CurrencyId,
			owner: T::AccountId,
		},
/// Directly close the ECDP that has debit by handle debit with DEX.
...
/// Runs after every block. Start offchain worker to check ECDP and
/// submit unsigned tx to trigger liquidation or settlement.
		fn offchain_worker(now: BlockNumberFor<T>) {
			if let Err(e) = Self::_offchain_worker() {
				log::info!(
					target: "ecdp-ussd-engine offchain worker",
...
/// Liquidate unsafe ECDP
///
/// The dispatch origin of this call must be _None_.
///
/// - `currency_id`: ECDP's collateral type.
/// - `who`: ECDP's owner.
		#[pallet::call_index(0)]
		#[pallet::weight(<T as Config>::WeightInfo::liquidate_by_auction(<T as Config>::EcdpUssdTreasury::max_auction()))]
		pub fn liquidate(
			origin: OriginFor<T>,
...
/// Settle ECDP that has debit after system shutdown
///
/// The dispatch origin of this call must be _None_.
///
/// - `currency_id`: ECDP's collateral type.
/// - `who`: ECDP's owner.
		...
		pub fn settle(
			...
			ensure_none(origin)?;
			let who = T::Lookup::lookup(who)?;
			ensure!(T::EcdpEmergencyShutdown::is_shutdown(), Error::<T>::MustAfterShutdown);
			Self::settle_cdp_has_debit(who, currency_id)?;
			Ok(())
		}

/// Update parameters related to risk management of ECDP under specific
/// collateral type
///
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			match call {
				Call::liquidate { currency_id, who } => {
					...
					let EcdpPosition { collateral, debit } = <LoansOf<T>>::positions(currency_id, &account);
					if !matches!(
						Self::check_cdp_status(*currency_id, collateral, debit),
						CDPStatus::Unsafe
					) || T::EcdpEmergencyShutdown::is_shutdown()
					{
						return InvalidTransaction::Stale.into();
					}

					ValidTransaction::with_tag_prefix("EcdpUssdEngineOffchainWorker")
				...
				Call::settle { currency_id, who } => {
					let account = T::Lookup::lookup(who.clone())?;
					let EcdpPosition { debit, .. } = <LoansOf<T>>::positions(currency_id, account);
					if debit.is_zero() || !T::EcdpEmergencyShutdown::is_shutdown() {
						return InvalidTransaction::Stale.into();
					}
...
// iterating from the beginning of <module_ecdp_loans::EcdpPositions<T>>
		...
		while let Some((who, EcdpPosition { collateral, debit })) = map_iterator.next() {
			if !is_shutdown
				&& matches!(
					Self::check_cdp_status(currency_id, collateral, debit),
					CDPStatus::Unsafe
...
// Consume the guard but **do not** unlock the underlying lock.
	...
	pub fn adjust_position_by_debit_value(
		...
		if debit_value_adjustment.is_negative() {
			let EcdpPosition { collateral: _, debit } = <LoansOf<T>>::positions(currency_id, who);
...
/// and the collateral ratio will be reduced but ECDP must still be at valid risk.
...
/// ECDP owner may receive some remainder assets.
	#[transactional]
...
// issue stable coin in advance
		<T as Config>::EcdpUssdTreasury::issue_debit(&loans_module_account, increase_debit_value, true)?;
...
// swap stable coin to collateral
				...
				let (_, actual_increase_collateral) =
					T::Swap::swap(&loans_module_account, T::GetUSSDCurrencyId::get(), currency_id, limit)?;
...
// update ECDP state
		...
		let EcdpPosition { collateral, debit } = <LoansOf<T>>::positions(currency_id, who);
// check the ECDP if is still at valid risk
...
/// debit are repaid, the extra stable coin will be transferred back to the ECDP
/// owner directly.
	#[transactional]
...
// ensure collateral of ECDP is enough
...
// refund extra stable coin to the ECDP owner
...
// repay the debit of ECDP
		<T as Config>::EcdpUssdTreasury::burn_debit(&loans_module_account, decrease_debit_value)?;

// check the ECDP if it is still at valid risk.
...
// and decrease ECDP's debit to zero
		let settle_price: Price = T::PriceSource::get_relative_price(T::GetUSSDCurrencyId::get(), currency_id)
...
// swap exact stable with DEX in limit of price impact
		...
		let (actual_supply_collateral, _) = <T as Config>::EcdpUssdTreasury::swap_collateral_to_stable(
			currency_id,
			SwapLimit::ExactTarget(collateral_supply, debit_value),
			false,
		)?;

// refund remainder collateral to ECDP owner
		...
		<T as Config>::EcdpUssdTreasury::withdraw_collateral(&who, currency_id, refund_collateral_amount)?;
...
// remove liquidity first
				let (amount_0, amount_1) =
					<T as Config>::EcdpUssdTreasury::remove_liquidity_for_lp_collateral(currency_id, collateral)?;
...
// these's stable refund
					if existing_stable > target_stable_amount {
						<T as Config>::EcdpUssdTreasury::withdraw_collateral(
							&who,
							stable_currency_id,
							existing_stable
...
// refund collateral to ECDP owner
			if !amount.is_zero() {
				<T as Config>::EcdpUssdTreasury::withdraw_collateral(who, currency_id, amount)?;
			}
			return Ok(());
		}
...
// calculate the supply limit by slippage limit for the price of oracle,
		let max_supply_limit = Ratio::one()
			...
			.saturating_mul_int(
				T::PriceSource::get_relative_price(T::GetUSSDCurrencyId::get(), currency_id)
		...
		let (actual_supply_collateral, actual_target_amount) = <T as Config>::EcdpUssdTreasury::swap_collateral_to_stable(
			currency_id,
...
// refund remainder collateral to ECDP owner
		...
		if actual_target_amount > target_stable_amount {
			<T as Config>::EcdpUssdTreasury::withdraw_surplus(
				who,
...
// sufficient repayment, transfer collateral to contract and notify
					if let Err(e) = <T as Config>::EcdpUssdTreasury::withdraw_collateral(
						&contract_account_id,
						currency_id,
						collateral_supply,
					) {
						log::error!(
							target: "ecdp-ussd-engine",
...
// refund the remaining collateral to ECDP owner
					let refund_collateral_amount = amount
					...
					if !refund_collateral_amount.is_zero() {
							...
							<T as Config>::EcdpUssdTreasury::withdraw_collateral(who, currency_id, refund_collateral_amount)
						{
							log::error!(
								target: "ecdp-ussd-engine",
								"LiquidateViaContracts: refund the remaining collateral to ECDP owner failed. \
...
impl<T: Config> LiquidateCollateral<T::AccountId> for LiquidateViaAuction<T> {
	fn liquidate(
		...
		<T as Config>::EcdpUssdTreasury::create_collateral_auctions(
			currency_id,
			amount,
			target_stable_amount,
			who.clone(),
...
impl<T: Config> EcdpUssdRiskManager<T::AccountId, CurrencyId, Balance, Balance> for Pallet<T> {
	...
	fn check_position_valid(
		...
		if !debit_balance.is_zero() {
			let debit_value = Self::get_debit_value(currency_id, debit_balance);
			let feed_price = <T as Config>::PriceSource::get_relative_price(currency_id, T::GetUSSDCurrencyId::get())
				.ok_or(Error::<T>::InvalidFeedPrice)?;
			let collateral_ratio =
