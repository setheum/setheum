...
extern crate sheyth_vm_linux_raw as linux_raw;
...
impl Drop for ChildProcess {
    fn drop(&mut self) {
        #[cfg(sheyth_vm_dev_debug_zygote)]
        let _ = self.send_signal(linux_raw::SIGINT);

        #[cfg(not(sheyth_vm_dev_debug_zygote))]
...
const ZYGOTE_BLOB_CONST: &[u8] = include_bytes!("./sheyth_vm-zygote");
...
fn prepare_zygote() -> Result<Fd, Error> {
    ...
    if cfg!(sheyth_vm_dev_debug_zygote) {
        let paths = [
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../sheyth_vm-zygote/target/x86_64-unknown-linux-gnu/debug/sheyth_vm-zygote"),
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../sheyth_vm-zygote/target/x86_64-unknown-linux-gnu/release/sheyth_vm-zygote"),
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/sandbox/sheyth_vm-zygote"),
            std::path::PathBuf::from("./sheyth_vm-zygote"),
        ];
    ...
    prepare_sealed_memfd(create_empty_memfd(cstr!("sheyth_vm_zygote"))?, length_aligned, [ZYGOTE_BLOB])
}
...
fn prepare_vmctx() -> Result<(Fd, Mmap), Error> {
    ...
    let memfd = create_empty_memfd(cstr!("sheyth_vm_vmctx"))?;
...
fn prepare_memory() -> Result<(Fd, Mmap), Error> {
    let memfd = create_empty_memfd(cstr!("sheyth_vm_memory"))?;
...
unsafe fn child_main(uid_map: &str, gid_map: &str, fds: ChildFds, sandboxing_enabled: bool) -> Result<(), Error> {
    ...
    pub use sheyth_vm_common::zygote;
    ...
    let child_argv: [*const u8; 2] = [cstr!("sheyth_vm-zygote").as_ptr().cast(), core::ptr::null()];
...
impl Sandbox {
    ...
    fn spawn_new_worker(global: &GlobalState, config: &SandboxConfig, worker_core: usize) -> Result<Box<Self>, Error> {
        ...
        let sandboxing_enabled = config.enable_sandboxing && !cfg!(sheyth_vm_dev_debug_zygote);
                            ...
                            Ok(count) => {
                                ...
                                while !tmp.is_empty() {
                                    if let Some(index) = tmp.iter().position(|&byte| byte == b'\n') {
                                        ...
                                        log::trace!(target: "sheyth_vm::zygote", "Child #{}: {}", child_pid, String::from_utf8_lossy(&buffer));
                                        buffer.clear();
                                    } else {
        ...
        fn wait_for_futex(vmctx: &VmCtx, child: &mut ChildProcess, current_state: u32, target_state: u32) -> Result<(), Error> {
            ...
            loop {
                ...
                if !cfg!(sheyth_vm_dev_debug_zygote) && instant.elapsed() > Duration::from_secs(10) {
