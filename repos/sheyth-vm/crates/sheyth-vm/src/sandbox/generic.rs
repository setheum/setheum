...
mod sys {
    pub use sheyth_vm_linux_raw::{c_int, c_void, siginfo_t, size_t, ucontext as ucontext_t, SIG_DFL, SIG_IGN};
    pub const SIGSEGV: c_int = sheyth_vm_linux_raw::SIGSEGV as c_int;
    pub const SIGILL: c_int = sheyth_vm_linux_raw::SIGILL as c_int;
    pub const PROT_READ: c_int = sheyth_vm_linux_raw::PROT_READ as c_int;
    pub const PROT_WRITE: c_int = sheyth_vm_linux_raw::PROT_WRITE as c_int;
    pub const PROT_EXEC: c_int = sheyth_vm_linux_raw::PROT_EXEC as c_int;
    pub const MAP_ANONYMOUS: c_int = sheyth_vm_linux_raw::MAP_ANONYMOUS as c_int;
    pub const MAP_PRIVATE: c_int = sheyth_vm_linux_raw::MAP_PRIVATE as c_int;
    pub const MAP_FIXED: c_int = sheyth_vm_linux_raw::MAP_FIXED as c_int;
    pub const MAP_FAILED: *mut c_void = !0 as *mut c_void;
    pub const SA_SIGINFO: c_int = sheyth_vm_linux_raw::SA_SIGINFO as c_int;
    pub const SA_NODEFER: c_int = sheyth_vm_linux_raw::SA_NODEFER as c_int;
    pub const MADV_DONTNEED: c_int = sheyth_vm_linux_raw::MADV_DONTNEED as c_int;
    pub const MADV_FREE: c_int = sheyth_vm_linux_raw::MADV_FREE as c_int;
...
unsafe extern "C" fn signal_handler(signal: c_int, info: &sys::siginfo_t, context: &sys::ucontext_t) {
    ...
    if !vmctx.is_null() {
        ...
        if vmctx.program_range.contains(&rip) {
            use sheyth_vm_common::regmap::NativeReg;
            for reg in sheyth_vm_common::program::Reg::ALL {
                let value = match sheyth_vm_common::regmap::to_native_reg(reg) {
            ...
            //
            sheyth_vm_common::static_assert!(sheyth_vm_common::regmap::TMP_REG.equals(NativeReg::rcx));
...
const REG_COUNT: usize = sheyth_vm_common::program::Reg::ALL.len();
...
sheyth_vm_common::static_assert!(core::mem::size_of::<VmCtx>() <= 4096);
...
impl super::Sandbox for Sandbox {
    ...
    fn spawn(_global: &Self::GlobalState, _config: &SandboxConfig, _outer_instance: Option<&Self>) -> Result<Box<Self>, Error> {
        ...
        sheyth_vm_common::static_assert!(GUEST_MEMORY_TO_VMCTX_OFFSET < 0);
