...
use sheyth_vm_common::abi::MemoryMapBuilder;
use sheyth_vm_common::cast::cast;
use sheyth_vm_common::program::{asm, InstructionSetKind};
use sheyth_vm_common::program::{BlobLen, Reg::*, INTERPRETER_CACHE_ENTRY_SIZE};
use sheyth_vm_common::utils::align_to_next_page_u32;
use sheyth_vm_common::writer::ProgramBlobBuilder;
use sheyth_vm_linker::TargetInstructionSet;
...
fn get_test_program(kind: TestProgram, is_64_bit: bool) -> &'static [u8] {
    ...
    let mut args = sheyth_vm_linker::TargetJsonArgs::default();
    args.is_64_bit = is_64_bit;
    args.rustc_version = sheyth_vm_linker::RustcVersion::Legacy;

    let (target, target_path) = if is_64_bit {
        ("riscv64emac-unknown-none-sheyth_vm", sheyth_vm_linker::target_json_path(args).unwrap())
    } else {
        ("riscv32emac-unknown-none-sheyth_vm", sheyth_vm_linker::target_json_path(args).unwrap())
    };
...
fn get_test_program(kind: TestProgram, is_64_bit: bool) -> &'static [u8] {
    match (kind, is_64_bit) {
        (TestProgram::Pinky, true) => include_bytes!("../../../guest-programs/target/riscv64emac-unknown-none-sheyth_vm/release/bench-pinky"),
        (TestProgram::Pinky, false) => {
            include_bytes!("../../../guest-programs/target/riscv32emac-unknown-none-sheyth_vm/release/bench-pinky")
        }
        (TestProgram::TestBlob, true) => include_bytes!("../../../guest-programs/target/riscv64emac-unknown-none-sheyth_vm/no-lto/test-blob"),
        (TestProgram::TestBlob, false) => {
            include_bytes!("../../../guest-programs/target/riscv32emac-unknown-none-sheyth_vm/no-lto/test-blob")
        }
        (TestProgram::TestBlobSo, true) => {
            include_bytes!("../../../guest-programs/target/riscv64emac-unknown-none-sheyth_vm/no-lto/test_blob.elf")
        }
        (TestProgram::TestBlobSo, false) => {
            include_bytes!("../../../guest-programs/target/riscv32emac-unknown-none-sheyth_vm/no-lto/test_blob.elf")
        }
    }
}
...
fn jump_into_middle_of_basic_block_from_within(engine_config: Config) {
    ...
    use sheyth_vm_common::program::{Instruction, ParsedInstruction};
...
fn jump_after_invalid_instruction_from_within(engine_config: Config) {
    ...
    assert_eq!(
        instructions[0],
        sheyth_vm_common::program::ParsedInstruction {
...
fn get_blob_impl(optimize: bool, strip: bool, elf: &'static [u8]) -> ProgramBlob {
    ...
    blob_map
        ...
        .or_insert_with(|| {
            ...
            let mut config = sheyth_vm_linker::Config::default();
            config.set_optimize(optimize);
            config.set_strip(strip);

            let bytes = sheyth_vm_linker::program_from_elf(config, TargetInstructionSet::Latest, &elf).unwrap();
...
fn access_memory_from_within(config: Config) {
    ...
    let mut spawn = move |code: &[sheyth_vm_common::program::Instruction]| -> crate::RawInstance {
        builder.set_code(code, &[]);
        let blob = ProgramBlob::parse(builder.to_vec().unwrap().into()).unwrap();
        let module = Module::from_blob(&engine, &module_config, blob).unwrap();
        let mut instance = module.instantiate().unwrap();
...
fn jam_validate_ok(config: Config) {
    ...
    let blob = ProgramBlob::parse(builder.into_vec().unwrap().into()).unwrap();
    assert!(blob.validate_code_with_isa(sheyth_vm_common::program::ISA_JamV1).is_ok());
...
fn jam_validate_invalid_opcode(config: Config) {
    ...
    raw_code[0] = 255;
    blob.set_code(raw_code.into());
    assert!(blob.validate_code_with_isa(sheyth_vm_common::program::ISA_JamV1).is_err());
    assert!(matches!(
...
fn jam_branch_target_just_past_the_end_of_code(config: Config) {
    ...
    assert!(matches!(
        ...
        sheyth_vm_common::program::Instruction::branch_eq_imm(_, _, target) if target == code_length + 2
    ));
...
fn test_basic_debug_info(raw_blob: &'static [u8]) {
    ...
    let frame = info
        .frames()
        .find(|frame| frame.kind() == sheyth_vm_common::program::FrameKind::Line)
        .unwrap();
...
fn test_advance_pc_and_const_add_pc_debug_info_64() {
    ...
    let bytes = sheyth_vm_linker::program_from_elf(config, TargetInstructionSet::ReviveV1, elf.as_slice());
    assert!(bytes.is_ok());
...
#[test]
fn blob_len_works() {
    const EXAMPLE_BLOB: &[u8] = include_bytes!("../../../guest-programs/output/example-hello-world.sheyth_vm");
...
fn run_riscv_test(engine_config: Config, elf: &[u8], testnum_reg: Reg, optimize: bool) {
    let _ = env_logger::try_init();
    let mut linker_config = sheyth_vm_linker::Config::default();
    linker_config.set_optimize(optimize);
    linker_config.set_strip(true);
    linker_config.set_min_stack_size(0);
    let raw_blob = sheyth_vm_linker::program_from_elf(linker_config, TargetInstructionSet::Latest, elf).unwrap();
