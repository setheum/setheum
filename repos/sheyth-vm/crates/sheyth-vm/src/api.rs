...
use sheyth_vm_common::abi::{MemoryMap, MemoryMapBuilder, VM_ADDR_RETURN_TO_HOST};
use sheyth_vm_common::cast::cast;
use sheyth_vm_common::program::{FrameKind, Imports, InstructionSetKind, Instructions, JumpTable, ProgramBlob, Reg};
use sheyth_vm_common::utils::{ArcBytes, AsUninitSliceMut, B32, B64};
...
if_compiler_is_supported! {
    #[cfg(target_os = "linux")]
    impl<T> IntoResult<T> for Result<T, sheyth_vm_linux_raw::Error> {
        fn into_result(self, message: &str) -> Result<T, Error> {
...
impl Module {
    ...
    pub(crate) fn find_start_of_basic_block(&self, offset: ProgramCounter) -> Option<ProgramCounter> {
        sheyth_vm_common::program::find_start_of_basic_block(
            self.state().blob.isa(),
    ...
    pub fn get_debug_string(&self, offset: u32) -> Result<&str, sheyth_vm_common::program::ProgramParseError> {
    ...
    pub fn from_blob(engine: &Engine, config: &ModuleConfig, blob: ProgramBlob) -> Result<Self, CompileError> {
        ...
        if engine.selected_backend == BackendKind::Interpreter && matches!(blob.isa(), InstructionSetKind::JamV1) {
            if let Err(pc) = blob.validate_code_with_isa(sheyth_vm_common::program::ISA_JamV1) {
        ...
        macro_rules! compile_module {
            ...
            ($sandbox_kind:ident, $bitness_kind:ident, $build_static_dispatch_table:ident, $visitor_name:ident, $module_kind:ident, $gas_kind:ty, $gas_kind_no_lifetime:ty, $gas_visitor:expr) => {{
                ...
                blob.visit(
                    sheyth_vm_common::program::$build_static_dispatch_table!($visitor_name, VisitorTy<'a>),
                    &mut visitor,
                );
        ...
        if_compiler_is_supported! {
            {
                match self.state().compiled_module {
                    #[cfg(target_os = "linux")]
                    CompiledModuleKind::Linux(..) => Some(sheyth_vm_common::zygote::VM_ADDR_NATIVE_CODE),
                    #[cfg(feature = "generic-sandbox")]
    ...
    pub(crate) fn display_instruction_at_impl(
        ...
        struct MaybeInstruction(Option<sheyth_vm_common::program::ParsedInstruction>, bool);
        impl core::fmt::Display for MaybeInstruction {
            fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
                if let Some(instruction) = self.0 {
                    let mut format = sheyth_vm_common::program::InstructionFormat::default();
...
impl RawInstance {
            ...
            if let Some(instruction) = self.module.instructions_bounded_at(program_counter).next() {
                ...
                if !self.module.is_dynamic_paging() {
                    ...
                    if offset >= aux_address {
                        ...
                        log::debug!("  Hint: try increasing your stack size with: 'sheyth_vm_derive::min_stack_size'");
                    }
                }
            }
        }
        ...
        B: ?Sized + AsUninitSliceMut,
    {
        ...
        if slice.is_empty() {
            ...
            unsafe {
                return Ok(sheyth_vm_common::utils::slice_assume_init_mut(slice));
            }
        }
