...
use sheyth_vm_common::abi::{MemoryMapBuilder, VM_CODE_ADDRESS_ALIGNMENT, VM_MAX_PAGE_SIZE, VM_MIN_PAGE_SIZE};
...
use sheyth_vm_common::utils::{align_to_next_page_u32, align_to_next_page_u64};
use sheyth_vm_common::varint;
use sheyth_vm_common::writer::{ProgramBlobBuilder, Writer};
...
impl From<sheyth_vm_common::program::Reg> for Reg {
    fn from(reg: sheyth_vm_common::program::Reg) -> Reg {
        use sheyth_vm_common::program::Reg as R;
        match reg {
            R::RA => Reg::RA,
            R::SP => Reg::SP,
            R::T0 => Reg::T0,
...
impl From<sheyth_vm_common::program::RawReg> for Reg {
    fn from(reg: sheyth_vm_common::program::RawReg) -> Reg {
        reg.get().into()
    }
}

impl From<sheyth_vm_common::program::RawReg> for RegImm {
    fn from(reg: sheyth_vm_common::program::RawReg) -> RegImm {
...
sheyth_vm_common::static_assert!(Reg::INPUT_REGS.len() == sheyth_vm_common::program::Reg::MAXIMUM_INPUT_REGS);
sheyth_vm_common::static_assert!(Reg::OUTPUT_REGS.len() == sheyth_vm_common::program::Reg::MAXIMUM_OUTPUT_REGS);
...
fn extract_exports(
    ...
    let mut b = sheyth_vm_common::elf::Reader::from(section.data());
...
mod test {
    ...
    struct TestProgram {
        disassembly: String,
        instance: sheyth_vm::RawInstance,
    }
    ...
    impl ProgramBuilder {
        ...
        fn append_assembly(&mut self, assembly: &str) {
            ...
            let raw_blob = sheyth_vm_common::assembler::assemble(Some(isa), assembly).unwrap();
            ...
            for instruction in blob.instructions() {
                ...
                match instruction.kind {
                    ...
                    Instruction::jump(target) => {
                        let target = *program_counter_to_section_target.get(&sheyth_vm::ProgramCounter(target)).unwrap();
                        *out = ControlInst::Jump { target }.into();
                    }
                    ...
                    Instruction::branch_less_unsigned_imm(src1, src2, target) | Instruction::branch_eq_imm(src1, src2, target) => {
                        let target_true = *program_counter_to_section_target.get(&sheyth_vm::ProgramCounter(target)).unwrap();
        ...
        fn build(&self, config: Config) -> TestProgram {
            ...
            config.set_backend(Some(sheyth_vm::BackendKind::Interpreter));
            let engine = sheyth_vm::Engine::new(&config).unwrap();
            let mut module_config = sheyth_vm::ModuleConfig::default();
            module_config.set_gas_metering(Some(sheyth_vm::GasMeteringKind::Sync));
            let module = sheyth_vm::Module::from_blob(&engine, &module_config, blob).unwrap();
            let mut instance = module.instantiate().unwrap();
            instance.set_gas(10000);
            instance.set_reg(sheyth_vm::Reg::RA, sheyth_vm::RETURN_TO_HOST);
        ...
        fn test_optimize(
            &self,
            mut run: impl FnMut(&mut sheyth_vm::RawInstance),
            mut check: impl FnMut(&mut sheyth_vm::RawInstance, &mut sheyth_vm::RawInstance),
        ...
        fn test_optimize_oneshot(
            assembly: &str,
            expected_disassembly: &str,
            run: impl FnMut(&mut sheyth_vm::RawInstance),
            check: impl FnMut(&mut sheyth_vm::RawInstance, &mut sheyth_vm::RawInstance),
        ) {
    ...
    fn expect_finished(i: &mut sheyth_vm::RawInstance) {
        assert!(matches!(i.run().unwrap(), sheyth_vm::InterruptKind::Finished));
    }

    fn expect_regs(regs: impl IntoIterator<Item = (Reg, u64)> + Clone) -> impl FnMut(&mut sheyth_vm::RawInstance, &mut sheyth_vm::RawInstance) {
        move |a: &mut sheyth_vm::RawInstance, b: &mut sheyth_vm::RawInstance| {
...
fn emit_code(
    ...
    use sheyth_vm_common::program::Reg as PReg;
    fn conv_reg(reg: Reg) -> sheyth_vm_common::program::RawReg {
        match reg {
            Reg::RA => PReg::RA,
            Reg::SP => PReg::SP,
            Reg::T0 => PReg::T0,
...
fn harvest_data_relocations(
    ...
    for (absolute_address, relocation) in section.relocations() {
        ...
        if relocation.flags()
            ...
            && section_name == ".sheyth_vm_exports"
        {
            relocations.insert(
                SectionTarget {
                    section_index: section.index(),
...
fn program_from_elf_internal(config: Config, isa: TargetInstructionSet, mut elf: Elf) -> Result<Vec<u8>, ProgramFromElfError> {
    ...
    for section in elf.sections() {
        ...
        } else if name == ".sheyth_vm_metadata" {
            sections_metadata.push(section.index());
        } else if name == ".sheyth_vm_exports" {
            sections_exports.push(section.index());
        } else if name == ".sheyth_vm_min_stack_size" {
    ...
    if isa.supports_opcode(Opcode::unlikely) && config.gas_cost_model_aware_optimizations {
        ...
        for (current, range) in used_blocks.iter().copied().zip(block_offsets.windows(2).map(|w| w[0]..w[1])) {
            use sheyth_vm_common::program::ParsedInstruction;
            use sheyth_vm_common::simulator::{CacheModel, Simulator};
            use sheyth_vm_common::utils::{GasVisitorT, B64};
...
mod test_debug_line_program {
    ...
    use sheyth_vm_common::program::{LineProgramConfig, ProgramParseError};
