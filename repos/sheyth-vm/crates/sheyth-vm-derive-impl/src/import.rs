...
fn parse_import_attributes(attr: &syn::Attribute) -> Result<Option<Vec<ImportAttribute>>, syn::Error> {
    if !is_path_eq(attr.meta.path(), "sheyth_vm_import") {
        return Ok(None);
    }
...
pub fn sheyth_vm_import(attributes: ImportBlockAttributes, input: syn::ItemForeignMod) -> Result<proc_macro2::TokenStream, syn::Error> {
        ...
        match item {
            syn::ForeignItem::Fn(syn::ForeignItemFn { attrs, sig, vis, .. }) => {
                ...
                tokens.push(quote! {
                    ...
                    #[link_section = ".text.sheyth_vm_import"]
