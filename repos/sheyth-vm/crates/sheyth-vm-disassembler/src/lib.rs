...
use sheyth_vm_common::program::{ParsedInstruction, ProgramBlob, ProgramCounter};
...
impl TryFrom<&'_ ProgramBlob> for NativeCode {
    ...
    fn try_from(blob: &'_ ProgramBlob) -> Result<Self, Self::Error> {
        ...
        let mut config = sheyth_vm::Config::from_env()?;
        config.set_worker_count(0);

        let engine = sheyth_vm::Engine::new(&config)?;
        let module = sheyth_vm::Module::from_blob(&engine, &Default::default(), blob.clone())?;
...
impl<'a> Disassembler<'a> {
    pub fn new(blob: &'a ProgramBlob, format: DisassemblyFormat) -> Result<Self, sheyth_vm::Error> {
    ...
    pub fn display_gas(&mut self) -> Result<(), sheyth_vm::Error> {
        let mut config = sheyth_vm::Config::from_env()?;
        config.set_worker_count(0);
        ...
        let mut config = sheyth_vm::ModuleConfig::default();
        config.set_gas_metering(Some(sheyth_vm::GasMeteringKind::Sync));

        let module = sheyth_vm::Module::from_blob(&engine, &config, self.blob.clone())?;
    ...
    pub fn disassemble_into(&self, mut writer: impl Write) -> Result<(), sheyth_vm::Error> {
        ...
        let mut disassembly_format = sheyth_vm_common::program::InstructionFormat::default();
        ...
        let jump_target_formatter = |target: u32, fmt: &mut core::fmt::Formatter| {
            if prefer_offset_jump_targets {
                write!(fmt, "{}", target)
            } else if let Some(basic_block_index) = instruction_offset_to_basic_block.get(&sheyth_vm::ProgramCounter(target)) {
        ...
        for (nth_instruction, instruction) in instructions.iter().copied().enumerate() {
            ...
            let instruction_s = if let sheyth_vm_common::program::Instruction::ecalli(nth_import) = instruction {
            ...
            if matches!(self.format, DisassemblyFormat::DiffFriendly) {
                ...
                if let sheyth_vm_common::program::Instruction::load_imm(dst, _) = instruction {
                    string = format!("{} = _", dst);
                }
