...
use crate::chain_spec::{
	...
	cli::ChainSpecParams,
	keystore::{create_account_session_keys, create_p2p_key, create_sbft_backup_dir, open_keystore},
	SetheumChainSpec,
};
...
pub struct BootstrapChainCmd {
	...
	/// `base-path` has an account id directory in it, and that directory stores keystore and db
	...
	/// Directory under which SetBFT backup is stored
	...
	pub fn run(&self) -> Result<(), Error> {
		...
		if authorities.is_empty() {
			if account_ids.is_empty() {
				return Err(("Both --account-ids and --authorities-account-ids are empty. \
                              Please specify at least one account.")
					.into());
			}
			authorities.clone_from(&account_ids);
		} else if !authorities.iter().all(|authority| account_ids.contains(authority)) {
			return Err("--authorities-account-ids must be a subset of --accounts-ids.".into());
		}

		let account_session_keys = self.chain_spec_params.clone().account_ids().into_iter().map(|account_id| {
			let account_base_path: BasePath = base_path.path().join(account_id.to_string()).into();
			create_sbft_backup_dir(account_base_path.path(), backup_dir);
		...
		let chain_spec = build_chain_spec_json(self.raw, &self.chain_spec_params, only_authorities)?;
...
impl ConvertChainspecToRawCmd {
	pub fn run(&self) -> Result<(), Error> {
		let spec = SetheumChainSpec::from_json_file(self.chain.to_owned()).expect("Cannot read chainspec");

		let raw_chainspec = sc_service::chain_ops::build_spec(&spec, true)?;
		if std::io::stdout().write_all(raw_chainspec.as_bytes()).is_err() {
			let _ = std::io::stderr().write_all(b"Error writing to stdout\n");
		}
