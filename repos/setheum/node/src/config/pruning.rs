...
impl PruningConfigValidator {
	fn pruning_changed(params: &PruningParams) -> bool {
		let state_pruning_changed = params.state_pruning.unwrap_or(DEFAULT_STATE_PRUNING) != DEFAULT_STATE_PRUNING;
	...
	pub fn process(cli: &mut Cli) -> Self {
		...
		let pruning_enabled = cli.setheum.enable_pruning();
	...
	pub fn report(self) {
		...
		if let Err((max_blocks, database)) = self.invalid_state_pruning_setting_for_rocksdb {
			let mut message = format!(
				...
				max_blocks, ROCKSDB_PRUNING_THRESHOLD, ROCKSDB_PRUNING_THRESHOLD
			)
			.to_string();
	...
	fn process_state_pruning(&mut self, cli: &mut Cli) {
		let database_engine = cli.run.import_params.database_params.database.unwrap_or(Database::Auto);
			...
			.get_or_insert(DatabasePruningMode::Custom(MINIMAL_STATE_PRUNING))
		{
			...
			DatabasePruningMode::Custom(max_blocks) => {
				if might_be_rocksdb && *max_blocks > ROCKSDB_PRUNING_THRESHOLD {
					self.invalid_state_pruning_setting_for_rocksdb = Err((*max_blocks, database_engine));
...
mod tests {
	...
	fn pruning_sanity_check() {
		let pruning_params =
			PruningParams { state_pruning: Some(DEFAULT_STATE_PRUNING), blocks_pruning: DEFAULT_BLOCKS_PRUNING };

		assert_eq!(pruning_params.blocks_pruning().unwrap(), BlocksPruning::KeepFinalized);

		assert_eq!(pruning_params.state_pruning().unwrap().unwrap(), PruningMode::ArchiveAll);
	}
}
