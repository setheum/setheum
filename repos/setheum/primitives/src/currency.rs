...
// SEU PAIRED POOLS
				Token {
					symbol: "LP_SEUSD_SEU".to_string(),
					address: EvmAddress::try_from(TradingPair::from_currency_ids(CurrencyId::Token(SEUSD), CurrencyId::Token(SEU)).unwrap().dex_share_currency_id()).unwrap(),
				},
			];
