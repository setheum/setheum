...
pub mod pallet {
    ...
    use super::*;
    use crate::{
        module_move_runtime_api::MoveApiEstimation,
        balance::{BalanceAdapter, BalanceOf},
        signer::*,
        storage::{MoveVmStorage, StorageAdapter},
        weight_info::WeightInfo,
        setheum::SetheumAdapter,
    };

    type MvmResult<T> = Result<Mvm<StorageAdapter<VMStorage<T>>, BalanceAdapter<T>, SetheumAdapter<T>>, Vec<u8>>;
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// NFT Class ID type
        type ClassId: Parameter + Member + Copy + MaybeSerializeDeserialize + Ord + TypeInfo + MaxEncodedLen;

        /// NFT Token ID type
        type TokenId: Parameter + Member + Copy + MaybeSerializeDeserialize + Ord + TypeInfo + MaxEncodedLen;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn move_vm() -> MvmResult<T> {
            ...
            let setheum = SetheumAdapter::new();

            Mvm::new(storage, balance, setheum)
        ...
        pub fn raw_execute_script(
            ...
            let vm = Mvm::new(storage, cheques, setheum).map_err(|_| Error::<T>::ExecuteFailed)?;
        ...
        pub fn raw_publish_module(
            ...
            let vm = Mvm::new(storage, BalanceAdapter::<T>::new(), setheum)
