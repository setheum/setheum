...
pub mod pallet {
	...
	use xcm::latest::{Junction, Asset, Location};
	...
	mod test {
		...
		use crate::mock::{
			...
			Runtime, RuntimeEvent, RuntimeOrigin, BridgeForwarder, UsdtAssetId, UsdtLocation,
		...
		fn test_xcm_transactor_forwarder() {
			new_test_ext().execute_with(|| {
				let asset: Asset =
					(Concrete(Location::new(0, Here)), Fungible(10u128)).into();
				let dest: Location =
					Location::new(1, X2(Parachain(1), GeneralIndex(1u128)));

				assert_ok!(BridgeForwarder::xcm_transactor_forwarder(
					ALICE.into(),
					asset.clone(),
					dest
				));

				assert_events(vec![RuntimeEvent::BridgeForwarder(
					BridgeForwarderEvent::XCMTransferForward {
						asset,
		...
		fn test_other_world_transactor_forwarder() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(BridgeForwarder::other_world_transactor_forwarder(
					ALICE.into(),
					asset.clone(),
					dest
				));
				...
				assert_events(vec![RuntimeEvent::BridgeForwarder(
					BridgeForwarderEvent::OtherWorldTransferForward {
						asset,
						origin: Junction::AccountId32 { network: None, id: ALICE.into() }.into(),
						dest,
					},
