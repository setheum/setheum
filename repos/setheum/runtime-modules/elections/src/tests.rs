...
use crate::{
	mock::{with_electable_targets, with_electing_voters, AccountId, Balance, Elections, Test, TestExtBuilder},
	CommitteeSize, CurrentEraValidators, NextEraCommitteeSize, NextEraNonReservedValidators, NextEraReservedValidators,
};
...
fn storage_is_initialized_already_in_genesis() {
	...
	const COMMITTEE_SEATS: CommitteeSeats =
		CommitteeSeats { reserved_seats: 3, non_reserved_seats: 2, non_reserved_finality_seats: 2 };
	...
	TestExtBuilder::new(RESERVED.to_vec(), NON_RESERVED.to_vec())
		...
		.execute_with(|| {
			...
			assert_eq!(CurrentEraValidators::<Test>::get().non_reserved, NON_RESERVED);
...
fn validators_are_elected_only_when_staking() {
	TestExtBuilder::new(vec![1, 2, 3, 4], vec![5, 6, 7, 8]).build().execute_with(|| {
		...
		assert_eq!(
			elected.into_inner(),
			&[(1, support(10, vec![(1, 10)])), (2, no_support()), (5, support(10, vec![(5, 10)])), (6, no_support()),]
		);
	});
}
