...
pub mod pallet {
	use frame_election_provider_support::{
		BoundedSupportsOf, DataProviderBounds, ElectionDataProvider, ElectionProvider, Support, Supports,
	};
	...
	#[pallet::storage]
	pub type NextEraCommitteeSize<T> = StorageValue<_, CommitteeSeats, ValueQuery, DefaultNextEraCommitteeSize<T>>;
	...
	pub type CurrentEraValidators<T: Config> = StorageValue<_, EraValidators<T::AccountId>, ValueQuery>;

	/// Next era's list of non reserved validators.
	#[pallet::storage]
	pub type NextEraNonReservedValidators<T: Config> = StorageValue<_, Vec<T::AccountId>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn change_validators(
			...
			let reserved_validators = reserved_validators.unwrap_or_else(NextEraReservedValidators::<T>::get);
			...
			Self::deposit_event(Event::ChangeValidators(reserved_validators, non_reserved_validators, committee_size));
		...
		#[pallet::call_index(4)]
		#[pallet::weight((T::BlockWeights::get().max_block, DispatchClass::Operational))]
		pub fn set_elections_openness(origin: OriginFor<T>, openness: ElectionOpenness) -> DispatchResult {
			ensure_root(origin)?;
	...
	impl<T: Config> Pallet<T> {
		fn ensure_validators_are_ok(
			...
			ensure!(committee_size_all <= validators_size, Error::<T>::NotEnoughValidators);

			ensure!(reserved <= reserved_len, Error::<T>::NotEnoughReservedValidators,);

			ensure!(non_reserved <= non_reserved_len, Error::<T>::NotEnoughReservedValidators,);

			let member_set: BTreeSet<_> = reserved_validators.into_iter().chain(non_reserved_validators).collect();

			ensure!(member_set.len() as u32 == validators_size, Error::<T>::NonUniqueListOfValidators);
	...
	impl<T: Config> ElectionProvider for Pallet<T> {
		...
		fn elect(_page: u32) -> Result<BoundedSupportsOf<Self>, Self::Error> {
			let staking_validators = Self::DataProvider::electable_targets(DataProviderBounds::default(), 0)
			...
			let staking_reserved_validators = NextEraReservedValidators::<T>::get()
				...
				.collect::<BTreeSet<_>>();
			let banned_validators = T::BannedValidators::banned().into_iter().collect::<BTreeSet<_>>();
			let old_non_reserved_validators = NextEraNonReservedValidators::<T>::get().into_iter();

			let eligible_non_reserved = staking_validators
				.into_iter()
				.filter(|v| !banned_validators.contains(v) && !staking_reserved_validators.contains(v))
				.collect::<BTreeSet<_>>();

			let new_non_reserved_validators: Vec<_> = match Openness::<T>::get() {
				ElectionOpenness::Permissioned => {
					old_non_reserved_validators.filter(|v| eligible_non_reserved.contains(v)).collect()
				},
			...
			let eligible_validators = staking_reserved_validators.into_iter().chain(new_non_reserved_validators);
			let mut supports = eligible_validators
				...
				.map(|id| {
					(
						...
						// is counted in.
						Support { total: 0, voters: Vec::new() },
					)
				})
			...
			supports.into_iter().collect::<Supports<_>>().try_into().map_err(|_| Self::Error::TooManyWinners)
		}
	}
}
