...
use primitives::{
	BanInfo, FinalityBanConfig as FinalityBanConfigStruct, ProductionBanConfig as ProductionBanConfigStruct,
...
pub(crate) const LOG_TARGET: &str = "module-committee-management";

#[frame_support::pallet]
#[pallet_doc("../README.md")]
pub mod pallet {
	use frame_support::{dispatch::DispatchResult, ensure, pallet_prelude::*, BoundedVec, Twox64Concat};
	use frame_system::{ensure_root, pallet_prelude::OriginFor};
	use primitives::{
		BanHandler, BanReason, BlockCount, FinalityCommitteeManager, SbftScoresProvider, SessionCount,
	...
	use module_traits::{EraInfoProvider, ValidatorExtractor, ValidatorRewardsHandler};
	...
	#[pallet::storage]
	pub type LenientThreshold<T: Config> = StorageValue<_, Perquintill, ValueQuery, DefaultLenientThreshold>;

	/// A lookup how many blocks a validator produced.
	#[pallet::storage]
	pub type SessionValidatorBlockCount<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, BlockCount, ValueQuery>;

	/// Total possible reward per validator for the current era.
	#[pallet::storage]
	pub type ValidatorEraTotalReward<T: Config> = StorageValue<_, ValidatorTotalRewards<T::AccountId>, OptionQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_ban_config(
			...
			if let Some(underperformed_session_count_threshold) = underperformed_session_count_threshold {
			...
			if let Some(clean_session_counter_delay) = clean_session_counter_delay {
				...
				current_committee_ban_config.clean_session_counter_delay = clean_session_counter_delay;
			}
		...
		pub fn set_finality_ban_config(
			...
			if let Some(minimal_expected_performance) = minimal_expected_performance {
				...
				current_committee_ban_config.minimal_expected_performance = minimal_expected_performance;
			}
