...
use crate::{
	mock::{
		active_era, advance_era, committee_management_events, start_session, AccountId, CommitteeManagement, Elections,
		SessionPeriod, TestBuilderConfig, TestExtBuilder, TestRuntime,
	},
...
fn new_poducers_every_session() {
	TestExtBuilder::new(gen_config()).build().execute_with(|| {
		...
		for session_index in 2..=6 {
			...
			let producers = CommitteeManagement::current_session_validators().current.producers.into_iter().collect();
...
fn new_finalizers_every_session() {
	TestExtBuilder::new(gen_config()).build().execute_with(|| {
		...
		for session_index in 2..=6 {
			start_session(session_index);
			let finalizers = CommitteeManagement::current_session_validators().current.finalizers.into_iter().collect();
...
fn all_reserved_validators_are_chosen() {
	TestExtBuilder::new(gen_config()).build().execute_with(|| {
		...
		start_session(2);
		let producers: BTreeSet<AccountId> =
			CommitteeManagement::current_session_validators().current.producers.into_iter().collect();
		assert!(reserved.iter().all(|rv| producers.contains(rv)));
		let finalizers: BTreeSet<AccountId> =
			CommitteeManagement::current_session_validators().current.finalizers.into_iter().collect();
...
fn ban_underperforming_producers() {
	TestExtBuilder::new(gen_config()).build().execute_with(|| {
		...
		let underperformed_session_count_threshold = ban_config.underperformed_session_count_threshold;
		let reserved: BTreeSet<AccountId> = Elections::current_era_validators().reserved.into_iter().collect();
		...
		loop {
			...
			for producer in producers.iter() {
				SessionValidatorBlockCount::<TestRuntime>::insert(producer, blocks_to_produce_per_session);
			}
		...
		let ban_info = BanInfo {
			reason: primitives::BanReason::InsufficientProduction(underperformed_session_count_threshold),
			start: active_era() + 1,
		};
		...
		assert_eq!(*committee_management_events().last().unwrap(), Event::BanValidators(banned_info));
		assert_eq!(CommitteeManagement::banned(), banned);
		advance_era();
		assert_eq!(CommitteeManagement::banned(), Vec::<AccountId>::new());
	})
...
fn ban_underperforming_finalizers() {
	TestExtBuilder::new(gen_config()).build().execute_with(|| {
		...
		loop {
			...
			let mut points: Vec<u16> = finalizers.iter().map(|_| minimal_expected_performance).collect();
			// Make sure underperformer underperforms.
			*points.last_mut().unwrap() = u16::MAX;
			let score = Score { session_id: session_index, nonce: 1, points };
		...
		assert_eq!(CommitteeManagement::banned(), banned);
		let ban_info = BanInfo {
			reason: primitives::BanReason::InsufficientFinalization(underperformed_session_count_threshold),
			start: active_era() + 1,
		};
