...
use primitives::{
	BanHandler, BanInfo, BanReason, BannedValidators, CommitteeSeats, EraValidators, SbftScoresProvider,
...
use crate::{
	pallet::{
		Banned, Config, CurrentAndNextSessionValidatorsStorage, Event, Pallet, SessionValidatorBlockCount,
		UnderperformedFinalizerSessionCount, UnderperformedValidatorSessionCount, ValidatorEraTotalReward,
	},
	traits::{EraInfoProvider, ValidatorRewardsHandler},
	CurrentAndNextSessionValidators, LenientThreshold, ProductionBanConfigStruct, ValidatorExtractor,
...
fn select_committee_inner<AccountId: Clone + PartialEq>(
	...
	let non_reserved_committee = choose_for_session(non_reserved, non_reserved_seats, current_session as usize);
...
fn calculate_adjusted_session_points(
	...
	let performance = Perquintill::from_rational(blocks_created as u64, blocks_to_produce_per_session as u64);
	...
	if performance >= lenient_threshold {
		return (Perquintill::from_rational(1, sessions_per_era as u64) * total_possible_reward as u64) as u32;
	}
...
pub fn compute_validator_scaled_total_rewards<V>(validator_totals: Vec<(V, u128)>) -> Vec<(V, u32)> {
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn blocks_to_produce_per_session() -> u32 {
		T::SessionPeriod::get().saturating_div(T::ValidatorProvider::current_era_committee_size().size())
	}

	pub fn adjust_rewards_for_session(underperf_finalizers: Vec<T::AccountId>) {
		let CurrentAndNextSessionValidators { current: SessionValidators { producers, non_committee, .. }, .. } =
		...
		let validator_total_rewards =
			ValidatorEraTotalReward::<T>::get().unwrap_or_else(|| ValidatorTotalRewards(BTreeMap::new())).0;
	...
	fn store_session_validators(
		...
		let non_committee = non_reserved.into_iter().chain(reserved).filter(|a| !producers_set.contains(a)).collect();
		...
		session_validators.next =
			SessionValidators { producers: producers.to_vec(), finalizers: finalizers.to_vec(), non_committee };
	...
	pub(crate) fn select_committee(
		...
		let EraValidators { reserved, non_reserved } = era_validators;

		let CommitteeSeats { reserved_seats, non_reserved_seats, non_reserved_finality_seats } = committee_seats;
	...
	pub(crate) fn calculate_underperforming_finalizers(session_id: SessionIndex) -> Vec<T::AccountId> {
		let CurrentAndNextSessionValidators { current: SessionValidators { finalizers, .. }, .. } =
			CurrentAndNextSessionValidatorsStorage::<T>::get();

		let finality_ban_config = Self::finality_ban_config();
		let underperformed_session_count_threshold = finality_ban_config.underperformed_session_count_threshold;
		...
		for (underperf, validator) in finalizers_perf.zip(finalizers.iter()) {
			if underperf {
				...
				let counter = UnderperformedFinalizerSessionCount::<T>::mutate(validator, |count| {
					*count += 1;
					*count
				});
	...
	pub(crate) fn calculate_underperforming_validators() {
		...
		let CurrentAndNextSessionValidators { current: SessionValidators { producers, .. }, .. } =
			CurrentAndNextSessionValidatorsStorage::<T>::get();
	...
	pub(crate) fn mark_validator_underperformance(thresholds: &ProductionBanConfigStruct, validator: &T::AccountId) {
	...
	pub fn ban_validator(validator: &T::AccountId, reason: BanReason) {
		...
		let start: EraIndex = T::EraInfoProvider::current_era().unwrap_or(0).saturating_add(1);
	...
	pub fn emit_fresh_bans_event() {
		...
		let fresh_bans = Banned::<T>::iter().filter(|(_acc, info)| info.start == active_era + 1).collect::<Vec<_>>();
	...
	pub fn predict_session_committee_for_session(
		...
		let ce =
			T::EraInfoProvider::current_era().ok_or_else(|| SessionValidatorError::Other("No current era".encode()))?;

		let current_starting_index = T::EraInfoProvider::era_start_session_index(ce)
			.ok_or_else(|| SessionValidatorError::Other("No known starting session for current era".encode()))?;
...
mod tests {
	...
	use crate::impls::{
		calculate_adjusted_session_points, compute_validator_scaled_total_rewards, select_committee_inner, MAX_REWARD,
	};
	...
	fn adjusted_session_points_all_blocks_created_are_calculated_correctly() {
		assert_eq!(5000, calculate_adjusted_session_points(5, 30, 30, 25_000, THRESHOLD));

		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 900, 600_000_000, THRESHOLD));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 900, 590_000_000, THRESHOLD));
	}

	#[test]
	fn adjusted_session_points_above_90_perc_are_calculated_correctly() {
		assert_eq!(5000, calculate_adjusted_session_points(5, 30, 27, 25_000, THRESHOLD));

		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 811, 600_000_000, THRESHOLD));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 899, 590_000_000, THRESHOLD));
	}

	#[test]
	fn adjusted_session_points_more_than_all_blocks_created_are_calculated_correctly() {
		assert_eq!(5000, calculate_adjusted_session_points(5, 30, 2 * 30, 25_000, THRESHOLD));

		assert_eq!(6250000, calculate_adjusted_session_points(96, 900, 3 * 900, 600_000_000, THRESHOLD));

		assert_eq!(6145833, calculate_adjusted_session_points(96, 900, 901, 590_000_000, THRESHOLD));
	}
	...
	fn scale_points_correctly_when_under_u32() {
		...
		assert_eq!(vec![(1, MAX_REWARD), (2, 0)], compute_validator_scaled_total_rewards(vec![(1, 10), (2, 0)]));
		assert_eq!(
			vec![(1, MAX_REWARD / 3), (2, MAX_REWARD / 6), (3, MAX_REWARD / 2),],
	...
	fn scale_points_correctly_when_above_u32() {
		...
		assert_eq!(vec![(1, MAX_REWARD), (2, 0)], compute_validator_scaled_total_rewards(vec![(1, 10 * max), (2, 0)]));
		assert_eq!(
			vec![(1, MAX_REWARD / 3), (2, MAX_REWARD / 6), (3, MAX_REWARD / 2),],
			compute_validator_scaled_total_rewards(vec![(1, 20 * max), (2, 10 * max), (3, 30 * max),])
		);
	}
