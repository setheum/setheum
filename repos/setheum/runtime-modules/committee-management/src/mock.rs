...
use primitives::{
	AuthorityId, CommitteeSeats, SessionIndex, SessionInfoProvider, TotalIssuanceProvider as TotalIssuanceProviderT,
	DEFAULT_MAX_WINNERS, DEFAULT_SESSIONS_PER_ERA, DEFAULT_SESSION_PERIOD,
};
...
use super::*;
use crate as module_committee_management;
...
impl SessionInfoProvider<BlockNumberFor<TestRuntime>> for SessionInfoImpl {
	...
	fn next_session_block_number(current_block: BlockNumberFor<TestRuntime>) -> Option<BlockNumberFor<TestRuntime>> {
		<TestRuntime as pallet_session::Config>::NextSessionRotation::estimate_next_session_rotation(current_block).0
	}
}
...
impl module_setbft::Config for TestRuntime {
...
impl module_elections::Config for TestRuntime {
	type RuntimeEvent = RuntimeEvent;
...
pub fn active_era() -> EraIndex {
	pallet_staking::ActiveEra::<TestRuntime>::get().unwrap().index
}
...
pub(crate) fn committee_management_events() -> Vec<crate::Event<TestRuntime>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let RuntimeEvent::CommitteeManagement(inner) = e { Some(inner) } else { None })
		.collect()
}
...
impl TestExtBuilder {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		let validators: Vec<_> = self.non_reserved_validators.iter().chain(self.reserved_validators.iter()).collect();
		...
		pallet_staking::GenesisConfig::<TestRuntime> {
			...
			canceled_payout: 0,
			stakers: validators.iter().map(|&&v| (v, v, 5_000_000, StakerStatus::<AccountId>::Validator)).collect(),
		...
		module_elections::GenesisConfig::<TestRuntime> {
