...
use crate as module_bridge_xcm;
...
construct_runtime!(
	pub struct Runtime {
		...
		ParachainInfo: module_bridge_parachain_info::{Pallet, Storage, Config<T>},

		XcmpQueue: cumulus_pallet_xcmp_queue::{Pallet, Call, Storage, Event<T>},
		CumulusXcm: cumulus_pallet_xcm::{Pallet, Event<T>, Origin},
		DmpQueue: cumulus_pallet_dmp_queue::{Pallet, Call, Storage, Event<T>},

		BridgeXcm: module_bridge_xcm::{Pallet, Event<T>},
	}
);
...
impl module_bridge_xcm::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
...
impl module_bridge_parachain_info::Config for Runtime {}
...
parameter_types! {
	...
	pub PAALocation: Location = Location::new(1, X1(Parachain(1u32)));
	pub PBALocation: Location = Location::new(1, X1(Parachain(2u32)));
...
parameter_types! {
	pub SelfLocation: Location = Location::new(1, X1(Parachain(ParachainInfo::parachain_id().into())));
...
impl WeightTrader for AllTokensAreCreatedEqualToWeight {
	...
	fn buy_weight(
		...
		if let Asset { fun: _, id: Concrete(ref id) } = &required {
			self.0 = *id;
		}
