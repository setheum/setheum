...
pub mod pallet {
	...
	use xcm::latest::{prelude::*, Location, Weight as XCMWeight};
	use xcm_executor::traits::WeightBounds;

	use module_bridge_traits::{AssetReserveLocationParser, AssetTypeIdentifier, Bridge};
	...
	impl<T: Config> Bridge for BridgeImpl<T> {
		fn transfer(sender: [u8; 32], asset: Asset, dest: Location, max_weight: Option<XCMWeight>) -> DispatchResult {
			...
			let fee_per_asset = T::MinXcmFee::get()
				...
				.map(|idx| T::MinXcmFee::get()[idx].1)
				.unwrap();
			let min_fee_to_dest: Asset = (asset.id, fee_per_asset).into();
	...
	impl<T: Config> Pallet<T> {
		...
		fn transfer_to_non_reserve_asset(
			...
			if reserve == Location::parent() {
				if let Location { parents: 1, interior: X1(Parachain(id)) } = dest {
