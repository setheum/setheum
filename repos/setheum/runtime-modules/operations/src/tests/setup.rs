...
impl pallet_staking::Config for TestRuntime {
	...
	type ElectionProvider = frame_election_provider_support::NoElection<(AccountId, u64, Staking, ConstU32<1>)>;
...
pub fn new_test_ext(accounts_and_balances: &[(u64, bool, u128)]) -> sp_io::TestExternalities {
	...
	let balances: Vec<_> = accounts_and_balances.iter().map(|(id, _, balance)| (*id, *balance)).collect();
	...
	pallet_staking::GenesisConfig::<TestRuntime> {
		validator_count: accounts_and_balances.iter().filter(|(_, is_authority, _)| *is_authority).count() as u32,
	...
	pallet_session::GenesisConfig::<TestRuntime> {
		keys: accounts_and_balances
			.iter()
			.filter(|(_, is_authority, _)| *is_authority)
			.map(|(id, _, _)| (*id, *id, TestSessionKeys { other: (*id).into() }))
			.collect(),
	}
	.assimilate_storage(&mut t)
	.unwrap();
