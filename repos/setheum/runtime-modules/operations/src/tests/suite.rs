...
fn pallet_operations_events() -> Vec<crate::Event<TestRuntime>> {
	frame_system::Pallet::<TestRuntime>::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let RuntimeEvent::Operations(inner) = e { Some(inner) } else { None })
		.collect()
}
...
fn given_accounts_with_initial_balance_then_balances_data_and_counters_are_valid() {
	...
	.execute_with(|| {
		...
		assert_eq!(usable_balance(non_authority_id), total_balance_non_authority);
...
fn given_accounts_with_initial_balance_when_reserving_then_balances_data_and_counters_are_valid() {
	...
	.execute_with(|| {
		...
		assert_ok!(pallet_balances::Pallet::<TestRuntime>::reserve(&non_authority_id, reserved_amount));
		assert_eq!(total_balance(non_authority_id), total_balance_non_authority);
		assert_eq!(free_balance(non_authority_id), total_balance_non_authority - reserved_amount);
		...
		// since consumers > 0
		assert_eq!(usable_balance(non_authority_id), total_balance_non_authority - reserved_amount - ed());
	});
}
...
fn given_account_with_initial_balance_when_bonding_then_balances_data_and_counters_are_valid() {
	...
	.execute_with(|| {
		...
		// free - max(frozen, ed)
		assert_eq!(usable_balance(non_authority_id), total_balance_non_authority - bonded);
		assert_eq!(providers(non_authority_id), 1);
...
fn given_accounts_with_initial_balance_when_fixing_consumers_then_counters_are_valid() {
	...
	.execute_with(|| {
		assert_eq!(consumers(authority_id), 3);
		assert_ok!(crate::Pallet::<TestRuntime>::fix_accounts_consumers_counter(
...
fn given_accounts_with_reserved_balance_when_fixing_consumers_then_counters_are_valid() {
	...
	.execute_with(|| {
		let reserved_amount = 3_u128;
		assert_ok!(pallet_balances::Pallet::<TestRuntime>::reserve(&authority_id, reserved_amount));
		assert_eq!(consumers(authority_id), 3);
		assert_ok!(crate::Pallet::<TestRuntime>::fix_accounts_consumers_counter(
			RuntimeOrigin::signed(authority_id),
			authority_id
...
fn given_account_zero_consumers_some_reserved_when_fixing_consumers_then_consumers_increase() {
	...
	.execute_with(|| {
		...
		assert_eq!(pallet_operations_events(), [crate::Event::ConsumersCounterIncremented { who: non_authority_id }]);
		assert_eq!(consumers(non_authority_id), 1);
	});
}
...
fn given_non_staking_account_with_vesting_lock_when_fixing_consumers_then_counters_are_valid() {
	...
	.execute_with(|| {
		assert_eq!(consumers(non_authority_id), 0);
		let locked = 3_u128;
		pallet_balances::Pallet::<TestRuntime>::set_lock(VESTING_ID, &non_authority_id, locked, WithdrawReasons::all());
		// +1 due to frozen > 0
...
fn given_validator_with_stash_equal_to_consumer_when_fixing_consumers_then_consumers_increases() {
	...
	.execute_with(|| {
		...
		assert_eq!(pallet_operations_events(), [crate::Event::ConsumersCounterIncremented { who: authority_id }]);
...
fn given_nominator_account_with_staking_lock_and_consumer_overflow_when_fixing_consumers_then_consumers_decrease() {
	...
	.execute_with(|| {
		...
		assert_eq!(pallet_operations_events(), [crate::Event::ConsumersCounterDecremented { who: authority_id }]);
