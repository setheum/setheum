...
frame_support::construct_runtime!(
	pub enum Runtime {
		...
		AccessSegregator: module_bridge_access_segregator::{Pallet, Call, Storage, Event<T>} = 4,
		BridgeBasicFeeHandler: module_bridge_basic_fee_handler::{Pallet, Call, Storage, Event<T>} = 5,
		Bridge: module_bridge::{Pallet, Call, Storage, Event<T>} = 6,
		BridgePercentageFeeHandler: module_bridge_percentage_fee_handler::{Pallet, Call, Storage, Event<T>} = 7,
		BridgeFeeHandlerRouter: module_bridge_fee_handler_router::{Pallet, Call, Storage, Event<T>} = 8,
	}
);
...
impl module_bridge_access_segregator::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type BridgeCommitteeOrigin = frame_system::EnsureRoot<Self::AccountId>;
	type PalletIndex = AccessSegregatorPalletIndex;
	type Extrinsics = RegisteredExtrinsics;
	type WeightInfo = module_bridge_access_segregator::weights::SygmaWeightInfo<Runtime>;
}

impl module_bridge_fee_handler_router::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	...
	type WeightInfo = module_bridge_fee_handler_router::weights::SygmaWeightInfo<Runtime>;
}

impl module_bridge_basic_fee_handler::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type PalletIndex = BaiscFeeHandlerPalletIndex;
	type WeightInfo = module_bridge_basic_fee_handler::weights::SygmaWeightInfo<Runtime>;
}

impl module_bridge_percentage_fee_handler::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type PalletIndex = PercentageFeeHandlerPalletIndex;
	type WeightInfo = module_bridge_percentage_fee_handler::weights::SygmaWeightInfo<Runtime>;
}
...
parameter_types! {
	...
	pub BridgeAccountNative: AccountId32 = BridgePalletId::get().into_account_truncating();
	pub BridgeAccountOtherTokens: AccountId32 = BridgePalletId::get().into_sub_account_truncating(1u32);
	...
	pub const BridgePalletId: PalletId = PalletId(*b"sygma/01");
}
...
impl module_bridge::Config for Runtime {
	...
	type FeeHandler = BridgeFeeHandlerRouter;
	...
	type WeightInfo = module_bridge::weights::SygmaWeightInfo<Runtime>;
}
