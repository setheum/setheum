...
use module_bridge_traits::{ChainID, DomainID, MpcAddress, ResourceId};

use crate::Pallet as Bridge;
use module_bridge_basic_fee_handler::Pallet as BasicFeeHandler;
use module_bridge_fee_handler_router::Pallet as FeeHandlerRouter;
...
#[benchmarks(
    where
		...
		T: module_bridge_basic_fee_handler::Config,
		T: module_bridge_fee_handler_router::Config,
...
mod benchmarks {
	...
	fn pause_all_bridges() {
		...
		for i in 1..domain_size + 1 {
			Bridge::<T>::register_domain(SystemOrigin::Root.into(), i, U256::from(i)).unwrap();
		}
	...
	fn unpause_all_bridges() {
		...
		for i in 1..domain_size + 1 {
			Bridge::<T>::pause_bridge(SystemOrigin::Root.into(), i).unwrap();
		}
