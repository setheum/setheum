...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + module_bridge_access_segregator::Config {
	...
	mod test {
		...
		fn get_token_reserved_account_test() {
			new_test_ext().execute_with(|| {
				assert_eq!(
					Bridge::get_token_reserved_account(&UsdtLocation::get().into()).unwrap(),
				...
				assert_eq!(
					Bridge::get_token_reserved_account(&AstrLocation::get().into()).unwrap(),
					BridgeAccountOtherTokens::get().as_slice()
				);
		...
		fn set_mpc_address() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::set_mpc_address(Origin::root(), test_mpc_addr_a));
				assert_eq!(MpcAddr::<Runtime>::get(), test_mpc_addr_a);

				// set to test_mpc_addr_b: should be MpcAddrNotUpdatable error
				assert_noop!(
					Bridge::set_mpc_address(Origin::root(), test_mpc_addr_b),
				...
				assert_noop!(
					Bridge::set_mpc_address(unauthorized_account, test_mpc_addr_a),
		...
		fn pause_bridge() {
			new_test_ext().execute_with(|| {
				...
				let unauthorized_account = Origin::from(Some(ALICE));
				assert_noop!(
					Bridge::pause_bridge(unauthorized_account, DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				assert!(IsPaused::<Runtime>::get(DEST_DOMAIN_ID));
			})
		...
		fn unpause_bridge() {
			new_test_ext().execute_with(|| {
				...
				assert_events(vec![RuntimeEvent::Bridge(BridgeEvent::BridgeUnpaused {
					dest_domain_id: DEST_DOMAIN_ID,
				})]);

				// try to unpause it again, should be error
				assert_noop!(
					Bridge::unpause_bridge(Origin::root(), DEST_DOMAIN_ID),
				...
				let unauthorized_account = Origin::from(Some(ALICE));
				assert_noop!(
					Bridge::unpause_bridge(unauthorized_account, DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				assert!(!IsPaused::<Runtime>::get(DEST_DOMAIN_ID));
			})
		...
		fn verify_mpc_signature_invalid_signature() {
			new_test_ext().execute_with(|| {
				...
				// should be false
				assert!(!Bridge::verify_by_mpc_address(final_message, signature.encode()));
			})
		}
		...
		fn deposit_native_asset_should_work() {
			new_test_ext().execute_with(|| {
				...
				// Check event
				assert_events(vec![
					RuntimeEvent::Bridge(BridgeEvent::Deposit {
						dest_domain_id: DEST_DOMAIN_ID,
		...
		fn transfer_wrap_deposit_should_work() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::transfer(ALICE.into(), asset.clone(), dest, None));
		...
		fn create_deposit_data_test() {
			new_test_ext().execute_with(|| {
				...
				let data = Bridge::create_deposit_data(100, recipient);
				// 32 + 32 + 42
		...
		fn deposit_unbounded_asset_should_fail() {
			new_test_ext().execute_with(|| {
				let unbounded_asset_location = Location::new(1, X1(GeneralIndex(123)));
				let test_mpc_addr: MpcAddress = MpcAddress([1u8; 20]);
				let fee = 100u128;
				let amount = 200u128;
		...
		fn retry_bridge() {
			new_test_ext().execute_with(|| {
				...
				assert_noop!(
					Bridge::retry(Origin::signed(ALICE), 1234567u128, DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				...
				assert_ok!(Bridge::retry(Origin::signed(ALICE), 1234567u128, DEST_DOMAIN_ID));
				assert_events(vec![RuntimeEvent::Bridge(BridgeEvent::Retry {
		...
		fn proposal_execution_should_work() {
			new_test_ext().execute_with(|| {
				// mpc address is missing, should fail
				assert_noop!(
					Bridge::execute_proposal(Origin::signed(ALICE), vec![], vec![]),
					bridge::Error::<Runtime>::MissingMpcAddress,
				);
				// set mpc address to generated keypair's address
				let (pair, _): (ecdsa::Pair, _) = Pair::generate();
				...
				let invalid_recipient_proposal = Proposal {
					...
					deposit_nonce: 3,
					resource_id: NativeResourceId::get(),
					data: Bridge::create_deposit_data(amount, b"invalid recipient".to_vec()),
				};
		...
		fn get_bridge_pause_status() {
			new_test_ext().execute_with(|| {
				assert!(!Bridge::is_paused(DEST_DOMAIN_ID));
		...
		fn access_control() {
			new_test_ext().execute_with(|| {
				...
				assert_noop!(
					Bridge::set_mpc_address(Some(ALICE).into(), test_mpc_addr),
					bridge::Error::<Runtime>::AccessDenied
				);

				assert_noop!(
					Bridge::pause_bridge(Some(BOB).into(), DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				assert_noop!(
					Bridge::unpause_bridge(Some(BOB).into(), DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				...
				assert_noop!(
					Bridge::set_mpc_address(Some(BOB).into(), test_mpc_addr),
					bridge::Error::<Runtime>::AccessDenied
				);
				...
				assert_noop!(
					Bridge::pause_bridge(Some(ALICE).into(), DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				assert_noop!(
					Bridge::unpause_bridge(Some(ALICE).into(), DEST_DOMAIN_ID),
					bridge::Error::<Runtime>::AccessDenied
				);
				// BOB pause&unpause bridge should work
				assert_ok!(Bridge::pause_bridge(Some(BOB).into(), DEST_DOMAIN_ID));
				assert_ok!(Bridge::unpause_bridge(Some(BOB).into(), DEST_DOMAIN_ID));
			})
		}
		...
		fn multi_domain_test() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::register_domain(Origin::root(), 1u8, U256::from(0)));
				...
				assert_noop!(
					Bridge::register_domain(Origin::from(Some(ALICE)), 1u8, U256::from(1)),
				...
				assert_noop!(
					Bridge::unregister_domain(Origin::from(Some(ALICE)), 1u8, U256::from(0)),
					Error::<Runtime>::AccessDenied
				);
				...
				// DestChainIDNotMatch
				assert_noop!(
					Bridge::unregister_domain(Origin::from(Some(ALICE)), 1u8, U256::from(2)),
					Error::<Runtime>::DestChainIDNotMatch
				);
				// alice unregister domainID 2 with chainID 2, should raise error
				// DestDomainNotSupported
				assert_noop!(
					Bridge::unregister_domain(Origin::from(Some(ALICE)), 2u8, U256::from(2)),
		...
		fn unpause_all_domains_test() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::pause_bridge(Some(ALICE).into(), 2));
				assert_ok!(Bridge::pause_bridge(Some(ALICE).into(), 3));

				// double check if they are all paused
				assert!(Bridge::is_paused(1));
				assert!(Bridge::is_paused(2));
				assert!(Bridge::is_paused(3));

				Bridge::unpause_all_domains();
		...
		fn deposit_native_asset_with_percentage_fee() {
			new_test_ext().execute_with(|| {
				...
				// test 5%
				assert_ok!(BridgePercentageFeeHandler::set_fee_rate(
					Origin::root(),
					DEST_DOMAIN_ID,
					Box::new(NativeLocation::get().into()),
					fee_rate_1,
				...
				assert_noop!(
					BridgePercentageFeeHandler::set_fee_rate(
						Origin::root(),
						DEST_DOMAIN_ID,
						Box::new(NativeLocation::get().into()),
						fee_rate_2,
					...
					module_bridge_percentage_fee_handler::Error::<Runtime>::FeeRateOutOfRange
				);
		...
		fn pause_all_bridges_test() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::register_domain(Origin::root(), domain_1, U256::from(1)));
				assert_ok!(Bridge::register_domain(Origin::root(), domain_2, U256::from(2)));
				...
				assert_ok!(Bridge::pause_all_bridges(Origin::signed(ALICE)));
		...
		fn unpause_all_bridges_test() {
			new_test_ext().execute_with(|| {
				...
				assert_ok!(Bridge::pause_bridge(Origin::root(), domain_1));
				assert_ok!(Bridge::pause_bridge(Origin::root(), domain_2));
				assert_ok!(Bridge::pause_bridge(Origin::root(), domain_3));
				...
				assert_ok!(Bridge::unpause_all_bridges(Origin::signed(ALICE)));
		...
		fn deposit_nonce_fix_should_work() {
			new_test_ext().execute_with(|| {
				...
				for nonce in 1..64u64 {
					Bridge::set_proposal_executed(nonce, 0);
				}
				// Nonce 0 should not be set
				assert!(!Bridge::is_proposal_executed(0, 0));
				// Nonce 1 should be set
				assert!(Bridge::is_proposal_executed(1, 0));
				// Nonce 63 should be set
				assert!(Bridge::is_proposal_executed(63, 0));
				...
				assert!(Bridge::is_proposal_executed(64, 0));
				// Nonce 127 should be set
				assert!(Bridge::is_proposal_executed(127, 0));
				// Nonce 128 should not be set
				assert!(!Bridge::is_proposal_executed(128, 0));
				...
				assert!(Bridge::is_proposal_executed(256, 0));
				// Nonce 300 should be set
				assert!(Bridge::is_proposal_executed(300, 0));
				// Nonce 301 should not be set
				assert!(!Bridge::is_proposal_executed(301, 0));
			})
		}
	}
}
