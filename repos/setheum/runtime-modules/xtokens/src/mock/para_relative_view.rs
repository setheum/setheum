...
use crate as module_xtokens;
use module_xtokens::ASSET_HUB_ID;
...
use module_traits::{location::Reserve, parameter_type_with_key};
use module_xcm_support::{IsNativeConcrete, MultiCurrencyAdapter};
...
impl module_xtokens::Config for Runtime {
	type Balance = Balance;
	type CurrencyId = CurrencyId;
...
impl module_xcm::Config for Runtime {
	type SovereignOrigin = EnsureRoot<AccountId>;
}

impl module_xcm_mock_message_queue::Config for Runtime {
	type XcmExecutor = XcmExecutor<XcmConfig>;
}
...
construct_runtime!(
	pub enum Runtime {
		...
		MsgQueue: module_xcm_mock_message_queue,
		CumulusXcm: cumulus_pallet_xcm,

		Tokens: module_tokens,
		XTokens: module_xtokens,

		PolkadotXcm: pallet_xcm,
		ModuleXcm: module_xcm,
	}
);
