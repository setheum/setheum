...
fn deposit_concrete_fungible_asset_works() {
	ExtBuilder.build().execute_with(|| {
		...
		assert_eq!(UnknownTokens::concrete_fungible_balances(&MOCK_RECIPIENT, &MOCK_CONCRETE_FUNGIBLE_ID), 3);
		System::assert_last_event(RuntimeEvent::UnknownTokens(crate::Event::Deposited { asset, who: MOCK_RECIPIENT }));

		// overflow case
		let max_asset = concrete_fungible(u128::max_value());
		assert_err!(UnknownTokens::deposit(&max_asset, &MOCK_RECIPIENT), Error::<Runtime>::BalanceOverflow);
	});
}
	...
	ExtBuilder.build().execute_with(|| {
		assert_err!(
			UnknownTokens::deposit(
				&Asset { fun: NonFungible(Undefined), id: AssetId(Location::parent()) },
				&MOCK_RECIPIENT
			),
...
fn withdraw_concrete_fungible_asset_works() {
	ExtBuilder.build().execute_with(|| {
		...
		assert_eq!(UnknownTokens::concrete_fungible_balances(&MOCK_RECIPIENT, &MOCK_CONCRETE_FUNGIBLE_ID), 0);
		...
		assert_err!(UnknownTokens::withdraw(&asset, &MOCK_RECIPIENT), Error::<Runtime>::BalanceTooLow);
	});
}
