...
fn enable_average_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexOracle::enable_average_price(RuntimeOrigin::signed(0), USSD, EDF, 0),
			BadOrigin
		);
		assert_noop!(
			DexOracle::enable_average_price(RuntimeOrigin::signed(1), USSD, LP_USSD_EDF, 0),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::enable_average_price(RuntimeOrigin::signed(1), USSD, EDF, 0),
			Error::<Runtime>::IntervalIsZero
		);
		assert_noop!(
			DexOracle::enable_average_price(RuntimeOrigin::signed(1), USSD, EDF, 12000),
			Error::<Runtime>::InvalidPool
		);

		set_pool(&USSDEDFPair::get(), 1_000, 100);
		assert_eq!(
			DexOracle::cumulatives(USSDEDFPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		assert_eq!(DexOracle::average_prices(USSDEDFPair::get()), None);
		...
		assert_eq!(
			DexOracle::average_prices(USSDEDFPair::get()),
			Some((
...
fn disable_average_price_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexOracle::disable_average_price(RuntimeOrigin::signed(0), USSD, EDF),
			BadOrigin
		);
		assert_noop!(
			DexOracle::disable_average_price(RuntimeOrigin::signed(1), USSD, LP_USSD_EDF),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::disable_average_price(RuntimeOrigin::signed(1), SEE, EDF),
			Error::<Runtime>::AveragePriceMustBeEnabled
		);

		assert_ok!(DexOracle::disable_average_price(RuntimeOrigin::signed(1), USSD, EDF));
		assert_eq!(
			DexOracle::cumulatives(USSDEDFPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
...
fn update_average_price_interval_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			DexOracle::update_average_price_interval(RuntimeOrigin::signed(0), USSD, EDF, 0),
			BadOrigin
		);
		assert_noop!(
			DexOracle::update_average_price_interval(RuntimeOrigin::signed(1), USSD, LP_USSD_EDF, 0),
			Error::<Runtime>::InvalidCurrencyId
		);
		assert_noop!(
			DexOracle::update_average_price_interval(RuntimeOrigin::signed(1), SEE, EDF, 0),
			Error::<Runtime>::AveragePriceMustBeEnabled
		);
		assert_noop!(
			DexOracle::update_average_price_interval(RuntimeOrigin::signed(1), USSD, EDF, 0),
			Error::<Runtime>::IntervalIsZero
		);
...
// will not cumulative if now is not gt than the last update cumulative timestamp.
		...
		DexOracle::try_update_cumulative(&USSDEDFPair::get(), 500, 200);
		assert_eq!(
			DexOracle::cumulatives(USSDEDFPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		...
		DexOracle::try_update_cumulative(&USSDEDFPair::get(), 1_000, 100);
		assert_eq!(
			DexOracle::cumulatives(USSDEDFPair::get()),
			(
...
// will not cumulative if TradingPair is not enabled as cumulative price.
		assert_eq!(
			DexOracle::cumulatives(SEEEDFPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
		DexOracle::try_update_cumulative(&SEEEDFPair::get(), 500, 200);
		assert_eq!(
			DexOracle::cumulatives(SEEEDFPair::get()),
			(U256::from(0), U256::from(0), 0)
		);
...
// initialize average prices
		assert_eq!(Timestamp::now(), 0);
		set_pool(&USSDEDFPair::get(), 1000, 100);
		assert_ok!(DexOracle::enable_average_price(
			RuntimeOrigin::signed(1),
			USSD,
			EDF,
		...
		set_pool(&SEEEDFPair::get(), 1000, 1000);
		assert_ok!(DexOracle::enable_average_price(
			RuntimeOrigin::signed(1),
			SEE,
			EDF,
		...
		assert_eq!(
			DexOracle::average_prices(SEEEDFPair::get()),
			Some((
...
// elapsed time is lt the update interval of USSD/EDF, update average price of USSD/EDF after try
// update cumulatives.
...
// elapsed time is lt the update interval of SEE/EDF, update average price of SEE/EDF after try
// update cumulatives.
		set_pool(&SEEEDFPair::get(), 1000, 2000);
		...
		set_pool(&USSDEDFPair::get(), 2000, 100);
		set_pool(&SEEEDFPair::get(), 1000, 4000);
...
// mock update cumulatives, the average prices are not updated.
		...
		DexOracle::try_update_cumulative(&USSDEDFPair::get(), 100, 100);
		DexOracle::try_update_cumulative(&SEEEDFPair::get(), 2000, 200);
		assert_eq!(
			DexOracle::cumulatives(USSDEDFPair::get()),
			(
...
// update average prices of USSD/EDF and SEE/EDF
		set_pool(&USSDEDFPair::get(), 1000, 100);
		set_pool(&SEEEDFPair::get(), 1000, 1000);
...
fn dex_price_providers_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(CurrentSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF), None);
		assert_eq!(CurrentSwapPriceProvider::<Runtime>::get_relative_price(EDF, USSD), None);
		assert_eq!(AverageSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF), None);
		assert_eq!(AverageSwapPriceProvider::<Runtime>::get_relative_price(EDF, USSD), None);
		assert_eq!(
			PriorityAverageSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF),
			None
		);
		assert_eq!(
			PriorityAverageSwapPriceProvider::<Runtime>::get_relative_price(EDF, USSD),
			None
		);

		set_pool(&USSDEDFPair::get(), 1_000, 100);
		assert_eq!(
			CurrentSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF),
			Some(ExchangeRate::saturating_from_rational(1, 10))
		);
		assert_eq!(
			CurrentSwapPriceProvider::<Runtime>::get_relative_price(EDF, USSD),
		...
		assert_eq!(
			AverageSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF),
			Some(ExchangeRate::saturating_from_rational(2, 10))
		);
		assert_eq!(
			AverageSwapPriceProvider::<Runtime>::get_relative_price(EDF, USSD),
		...
		set_pool(&USSDEDFPair::get(), 300, 100);
		assert_eq!(
			CurrentSwapPriceProvider::<Runtime>::get_relative_price(USSD, EDF),
