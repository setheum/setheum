...
/// CurrentSwapPriceProvider that always provider real-time prices from dex.
pub struct CurrentSwapPriceProvider<T>(PhantomData<T>);
impl<T: Config> SwapPriceProvider<CurrencyId> for CurrentSwapPriceProvider<T> {
...
/// AverageSwapPriceProvider that always provider average price.
pub struct AverageSwapPriceProvider<T>(PhantomData<T>);
impl<T: Config> SwapPriceProvider<CurrencyId> for AverageSwapPriceProvider<T> {
	fn get_relative_price(base: CurrencyId, quote: CurrencyId) -> Option<ExchangeRate> {
		let trading_pair = TradingPair::from_currency_ids(base, quote)?;
...
/// PriorityAverageSwapPriceProvider that priority access to the average price, if it is none,
/// will access to real-time price from dex.
pub struct PriorityAverageSwapPriceProvider<T>(PhantomData<T>);
impl<T: Config> SwapPriceProvider<CurrencyId> for PriorityAverageSwapPriceProvider<T> {
	fn get_relative_price(base: CurrencyId, quote: CurrencyId) -> Option<ExchangeRate> {
		let trading_pair = TradingPair::from_currency_ids(base, quote)?;
