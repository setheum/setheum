	...
	mod test {
		...
		fn set_get_fee() {
			new_test_ext().execute_with(|| {
				...
				let asset_id_a = Concrete(Location::new(1, Here));
				let asset_id_b = Concrete(Location::new(2, Here));
				let asset_a_deposit: Asset = (asset_id_a, 100u128).into();
				let asset_b_deposit: Asset = (asset_id_b, 200u128).into();
