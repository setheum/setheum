...
use setheum_client::{
	...
	waiting::{BlockStatus, SetBFTWaiting},
	AccountId, Balance, CodeHash, Connection, SignedConnection, SignedConnectionApi, TxStatus,
};

use crate::{
	commands::{
		ContractCall, ContractCodeInfo, ContractInstantiate, ContractInstantiateWithCode, ContractOptions,
...
pub async fn upload_code(
	...
	let ContractUploadCode { wasm_path, storage_deposit_limit } = command;
	...
	let _tx_info = signed_connection
		.upload_code(
			...
			setheum_client::pallet_contracts::wasm::Determinism::Enforced,
			TxStatus::InBlock,
		)
		.await?;
...
pub async fn instantiate(
	...
	let ContractInstantiate { code_hash, metadata_path, constructor, args, options } = command;

	let ContractOptions { balance, gas_limit, storage_deposit_limit } = options;
...
pub async fn instantiate_with_code(
	...
	let ContractInstantiateWithCode { wasm_path, metadata_path, constructor, args, options } = command;
...
pub async fn call(signed_connection: SignedConnection, command: ContractCall) -> anyhow::Result<()> {
	let ContractCall { destination, message, args, metadata_path, options } = command;
...
pub async fn remove_code(
	...
	let _tx_info = signed_connection.remove_code(code_hash, TxStatus::InBlock).await?;
...
fn load_metadata(path: &Path) -> anyhow::Result<ink_metadata::InkProject> {
	...
	let metadata: ContractMetadata = serde_json::from_reader(file).expect("Failed to deserialize metadata file");
