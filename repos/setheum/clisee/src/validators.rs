...
use setheum_client::{pallets::elections::ElectionsSudoApi, primitives::CommitteeSeats, RootConnection, TxStatus};

use crate::commands::ChangeValidatorArgs;

/// Change validators to the provided list by calling the provided node.
pub async fn change_validators(root_connection: RootConnection, change_validator_args: ChangeValidatorArgs) {
	root_connection
		.change_validators(
			change_validator_args.reserved_validators,
			change_validator_args.non_reserved_validators,
			change_validator_args.committee_size.map(|s| CommitteeSeats {
