...
use primitives::{
	crypto::SignatureSet, AccountId, ApiError as SetBFTApiError, AuraId, AuthorityId as SetBFTId, AuthoritySignature,
	Balance, Block, Nonce, Perbill, Score, SessionAuthorityData, SessionCommittee, SessionIndex, SessionValidatorError,
	...
	sp_api::impl_runtime_apis! {
		...
		> for Runtime {
		 ...
		 impl crate::SetBFTSessionApi<Block> for Runtime {
			...
			fn authorities() -> Vec<SetBFTId> {
				unimplemented!()
			}

			fn next_session_authorities() -> Result<Vec<SetBFTId>, SetBFTApiError> {
				unimplemented!()
			}
			...
			fn next_session_authority_data() -> Result<SessionAuthorityData, SetBFTApiError> {
				unimplemented!()
			}
