	...
	TP: TimeProvider,
{
	...
	fn available(&self) -> Option<u64> {
		(self.requested <= self.upper_bound_of_tokens()).then(|| self.upper_bound_of_tokens() - self.requested)
	}
	...
	fn calculate_delay(&self) -> Option<Instant> {
		...
		let delay_millis = scheduled_for_later.saturating_mul(1_000).saturating_div(self.rate_per_second.into());
...
impl SharedBandwidthManager {
	...
	pub fn new(max_rate: NonZeroRatePerSecond) -> Self {
		Self { max_rate, peers_count: Arc::new(AtomicU64::new(0)), already_requested: None }
	}

	pub fn share(&self) -> Self {
		Self { max_rate: self.max_rate, peers_count: self.peers_count.clone(), already_requested: None }
	}

	fn calculate_bandwidth(&mut self, active_children: Option<u64>) -> NonZeroRatePerSecond {
		let active_children = active_children.unwrap_or_else(|| self.peers_count.load(Ordering::SeqCst));
		let rate = u64::from(self.max_rate) / active_children;
		NonZeroU64::try_from(rate).map(NonZeroRatePerSecond::from).unwrap_or(MIN)
	}

	/// Allocate part of the shared bandwidth.
	pub fn request_bandwidth(&mut self) -> NonZeroRatePerSecond {
		let active_children =
			(self.already_requested.is_none()).then(|| 1 + self.peers_count.fetch_add(1, Ordering::SeqCst));
	...
	TP: TimeProvider,
{
	...
	pub fn new(token_bucket: TokenBucket<TP>, sleep_until: SU) -> Self {
		Self { token_bucket, next_deadline: None, sleep_until }
	}
...
impl<TP, SU> SharedTokenBucket<TP, SU> {
	fn new_internal(rate: NonZeroRatePerSecond, rate_limiter: AsyncTokenBucket<TP, SU>) -> Self {
		Self { shared_bandwidth: SharedBandwidthManager::new(rate), rate_limiter, need_to_notify_parent: false }
	}
...
mod tests {
		...
		TP: TimeProvider + Send,
	{
		async fn rate_limit(mut self, requested: u64) -> (Self, Option<Instant>) {
			...
			(self, (time_before != time_after).then_some(time_after).flatten())
		}
	}
	...
	impl SharedTracingSleepUntil {
		pub fn new() -> Self {
			Self { last_deadline: Arc::new(Mutex::new(None)) }
		}
	}
	...
	where
		RL: RateLimiter
			+ From<(NonZeroRatePerSecond, Arc<Box<dyn TimeProvider + Send + Sync>>, SharedTracingSleepUntil)>,
	{
		...
		let time_provider: Box<dyn TimeProvider + Send + Sync> = Box::new(move || *time_provider.read());
		let rate_limiter = RL::from((limit_per_second, Arc::new(time_provider), SharedTracingSleepUntil::new()));
			...
			+ From<(NonZeroRatePerSecond, Arc<Box<dyn TimeProvider + Send + Sync>>, SharedTracingSleepUntil)>,
	{
		...
		*time_to_return.write() = now + Duration::from_secs(1);
		...
		assert_eq!(deadline, Some(now + Duration::from_secs(3)), "we should wait exactly 2 seconds");
	}
			...
			+ From<(NonZeroRatePerSecond, Arc<Box<dyn TimeProvider + Send + Sync>>, SharedTracingSleepUntil)>,
	{
		...
		let time_provider = time_to_return.clone();
		let time_provider: Box<dyn TimeProvider + Send + Sync> = Box::new(move || *time_provider.read());
		let rate_limiter = RL::from((limit_per_second, time_provider.into(), SharedTracingSleepUntil::new()));
		...
		*time_to_return.write() = now + Duration::from_secs(10);
		...
		assert_eq!(deadline, Some(now + Duration::from_secs(10) + Duration::from_secs(3)),);

		*time_to_return.write() = now + Duration::from_secs(11);
		let (_, deadline) = rate_limiter.rate_limit(40).await;
		assert_eq!(deadline, Some(now + Duration::from_secs(11) + Duration::from_secs(6)));
	}
			...
			+ From<(NonZeroRatePerSecond, Arc<Box<dyn TimeProvider + Send + Sync>>, SharedTracingSleepUntil)>,
	{
		...
		*time_to_return.write() = now + Duration::from_secs(3);
		let (rate_limiter, deadline) = rate_limiter.rate_limit(10).await;
		assert_eq!(deadline, Some(now + Duration::from_secs(4) + Duration::from_secs(1)));

		*time_to_return.write() = now + Duration::from_secs(3);
		let (_, deadline) = rate_limiter.rate_limit(50).await;
		assert_eq!(deadline, Some(now + Duration::from_secs(4) + Duration::from_secs(6)));
	}
	...
	impl<SU> SleepUntilAfterChange<SU> {
		pub fn new(sleep_until: SU) -> Self {
			Self { wrapped: sleep_until, last_value: None, changes_counter: 0 }
		}
	...
	{
		fn clone(&self) -> Self {
			Self { wrapped: self.wrapped.clone(), last_value: None, changes_counter: self.changes_counter }
		}
	}
	...
	async fn two_peers_can_share_bandwidth() {
		...
		let mut rate_limiter = SharedTokenBucket::<_, _>::from((limit_per_second, time_provider, sleep_until));
		...
		let total_data_sent = thread::scope(|s| {
			let first_handle = s.spawn(|| {
				...
				runtime.block_on(async move {
					...
					rate_limiter.rate_limiter.sleep_until.set_number_of_changes_to_wait(1);
			...
			let second_handle = s.spawn(|| {
				...
				runtime.block_on(async {
					...
					rate_limiter_cloned.rate_limiter.sleep_until.set_number_of_changes_to_wait(1);
			...
			let total_data_sent: u128 = first_handle.join().expect("first thread should finish without errors")
				+ second_handle.join().expect("second thread should finish without errors");
	...
	async fn single_peer_can_use_whole_bandwidth() {
		...
		let time_to_return = Arc::new(parking_lot::RwLock::new(now));
		let time_provider = time_to_return.clone();
		let time_provider: Arc<Box<dyn TimeProvider + Send + Sync>> = Arc::new(Box::new(move || *time_provider.read()));

		let rate_limiter =
			TracingRateLimiter::<_>::from((limit_per_second, time_provider, SharedTracingSleepUntil::new()));
	...
	async fn avarage_bandwidth_should_be_within_some_reasonable_bounds() {
		...
		let seed = SystemTime::now().duration_since(UNIX_EPOCH).expect("back to the future").as_secs();
		...
		let sleep_until_after_time_change = SleepUntilAfterChange::new(test_sleep_until_with_barrier);
		let rate_limiter =
			SharedTokenBucket::<_, _>::from((limit_per_second, time_provider, sleep_until_after_time_change));
		...
		while total_number_of_calls < 1000 {
			...
			*barrier.write().await = tokio::sync::Barrier::new(batch_size);
			...
			let mut batch_test: FuturesOrdered<_> = rate_limiters[0..batch_size]
				...
				.map(|((selected_limiter_id, selected_rate_limiter), idx)| {
					...
					let mut rate_limiter =
						selected_rate_limiter.take().expect("we should be able to retrieve a rate-limiter");

					// last instance won't be notified - its bandwidth will not change until some other instance finishes
					rate_limiter.rate_limiter.sleep_until.set_number_of_changes_to_wait(min(1, idx));
			...
			let current_time = max(*time_to_return.read(), (*last_deadline.lock()).unwrap_or(*time_to_return.read()));
			...
			let max_possible_bandwidth = rate_limit as f64 * ((batch_size as f64).ln() + 1_f64).trunc();
			assert!(
