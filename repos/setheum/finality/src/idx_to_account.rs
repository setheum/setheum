...
use primitives::{AccountId, SetBFTSessionApi, AuraId, BlockHash, BlockNumber};
    ...
    RA: RuntimeApi,
{
    fn account(&self, session: SessionId, validator_index: NodeIndex) -> Option<AccountId> {
        ...
        let setbft_key = authority_data.authorities()[validator_index.0].clone();
        self.client
            .runtime_api()
            .key_owner(block_hash, setbft_key)
            .ok()?
    }
}
