...
#[derive(PartialEq, Eq, Clone, Debug)]
pub struct PendingMessageInfo<M: SetBFTNetworkMessage> {
    message: M,
// Data items that we still wait for
    pending_proposals: HashSet<SetBFTProposal>,
}

impl<M: SetBFTNetworkMessage> PendingMessageInfo<M> {
    ...
    R: Receiver<Message>,
{
    ...
    pending_proposals: HashMap<SetBFTProposal, PendingProposalInfo>,
    event_triggers: HashMap<ChainEvent, HashSet<SetBFTProposal>>,
...
// when pruning messages.
    ...
    available_proposals_cache: LruCache<SetBFTProposal, ProposalStatus>,
...
// proposal (with not finalized "base") or because we are not up-to-date with finalization.
    ...
    fn register_block_import_trigger(&mut self, proposal: &SetBFTProposal, block: &BlockId) {
    ...
    fn register_finality_trigger(&mut self, proposal: &SetBFTProposal, number: BlockNumber) {
        self.num_triggers_registered_since_last_pruning += 1;
    ...
    fn register_next_finality_trigger(&mut self, proposal: &SetBFTProposal) {
...
// We activate all finality triggers in [old_num + 1, block.num].
    ...
    fn on_proposal_available(&mut self, proposal: &SetBFTProposal) {
...
// this proposal becoming available.
    fn bump_proposal(&mut self, proposal: &SetBFTProposal) -> bool {
...
// proposals a message waits for.
    fn remove_proposal_from_pending_message(&mut self, proposal: &SetBFTProposal, id: MessageId) {
...
// We reinsert the message because it still has pending proposals.
    ...
    fn remove_message_id_from_pending_proposal(&mut self, proposal: &SetBFTProposal, id: MessageId) {
