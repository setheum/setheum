...
use crate::{
    ...
    data_io::legacy::{proposal::UnvalidatedSetBFTProposal, SetBFTData, MAX_DATA_BRANCH_LEN},
...
// The hashes in `branch` are ordered from top to bottom -- need to reverse.
        branch.reverse();
        Ok(SetBFTData {
            head_proposal: UnvalidatedSetBFTProposal::new(branch, num_last),
        })
    } else {
    ...
    BBS: BestBlockSelector<H> + 'static,
{
    ...
    client: C,
    data_to_propose: Arc<Mutex<Option<SetBFTData>>>,
...
impl DataProvider {
    pub async fn get_data(&mut self) -> Option<SetBFTData> {
...
// This theoretically might fail, but practically shouldn't.
    async fn sleep_until_data_available(data_provider: &mut DataProvider) -> SetBFTData {
