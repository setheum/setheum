...
impl<UH: UnverifiedHeader> legacy_set_bft::DataProvider<SetBFTData<UH>> for DataProvider<UH> {
    async fn get_data(&mut self) -> Option<SetBFTData<UH>> {
        DataProvider::get_data(self).await
    }
}

impl<CIP, H, V> legacy_set_bft::FinalizationHandler<SetBFTData<H::Unverified>>
