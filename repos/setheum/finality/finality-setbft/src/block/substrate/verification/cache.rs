...
use crate::{
    setbft_primitives::{AccountId, AuraId, Block, BlockNumber, Header, MILLISECS_PER_BLOCK},
    block::{
        substrate::{
            verification::{
    ...
    FS: FinalizationInfo,
{
    ...
    fn verify_justification(
        ...
        match &justification.inner_justification {
            InnerJustification::SetBFTJustification(setbft_justification) => {
                let verifier = self.get_verifier(*header.number())?;
                verifier.verify_bytes(setbft_justification, header.hash().encode())?;
                Ok(justification)
            }
...
mod tests {
    ...
    use crate::{
        setbft_primitives::SessionAuthorityData,
