    ...
    fn process_equivocation_proofs<I: IntoIterator<Item = V::EquivocationProof>>(&self, proofs: I) {
        for proof in proofs {
            ...
            if proof.are_we_equivocating() {
                panic!("We are equivocating, which is ILLEGAL - shutting down the node. This is probably caused by running two instances of the node with the same set of credentials. Make sure that you are running ONLY ONE instance of the node. If the problem persists, contact the SetBFT Zero developers on Discord.");
            }
        }
    }
