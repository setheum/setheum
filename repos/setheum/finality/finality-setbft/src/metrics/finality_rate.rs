    ...
    pub fn new(registry: Option<&Registry>) -> Result<Self, PrometheusError> {
        ...
        Ok(FinalityRateMetrics::Prometheus {
            own_finalized: register(
                Counter::new("setbft_own_finalized_blocks", "Number of self-produced blocks that became finalized")?,
                registry,
            )?,
            own_hopeless: register(
                Counter::new("setbft_own_hopeless_blocks", "Number of self-produced blocks, such that some alternative block with the same block number was finalized")?,
                registry,
            )?,
