...
use setheum_client::pallets::author::AuthorRpc;
use setheum_client::pallets::elections::ElectionsSudoApi;
use setheum_client::pallets::session::SessionUserApi;
use setheum_client::primitives::CommitteeSeats;
use setheum_client::{
	keypair_from_string,
	pallets::{
	...
	AccountId, Balance, ConnectionApi, KeyPair, RootConnection, SignedConnection, SignedConnectionApi, TxStatus,
};
...
async fn main() -> anyhow::Result<()> {
	...
	let Config { address, root_seed_file, validators_seed_file, validator_count } = Config::parse();
	...
	let validators = match validators_seed_file {
		Some(validators_seed_file) => {
			...
			validators_seeds.split('\n').filter(|seed| !seed.is_empty()).map(keypair_from_string).collect()
		},
		None => (0..validator_count.unwrap()).map(derive_user_account_from_numeric_seed).collect::<Vec<_>>(),
	};
	let validator_count = validators.len() as u32;
	warn!(
		"Make sure you have exactly {} nodes run in the background, otherwise you'll see extrinsic send failed errors.",
		validator_count
	);
	...
	bond_validators_funds_and_rotate_keys(
		"ws://127.0.0.1",
		controllers.iter().map(|k| KeyPair::new(k.signer().clone())).collect(),
	)
	.await;
	info!("wait until our node is forced to use new keys, i.e. current session + 2");
	root_connection.wait_for_n_sessions(2, BlockStatus::Best).await;
	...
	wait_for_successive_eras(&address, &root_connection, validators_and_nominator_stashes, ERAS_TO_WAIT).await?;
...
fn generate_controllers_for_validators(validator_count: u32) -> Vec<KeyPair> {
	(0..validator_count).map(|seed| keypair_from_string(&format!("//{seed}"))).collect::<Vec<_>>()
}
...
async fn setup_test_validators_and_nominator_stashes(
	...
	for (validator_index, validator) in validators.into_iter().enumerate() {
		let nominator_stash_accounts =
			generate_nominator_accounts_with_minimal_bond(connection, validator_index as u32, validators_len as u32)
				.await;
		let nominee_account = validator.account_id().clone();
		info!("Nominating validator {}", nominee_account);
		nominate_validator(connection, nominator_stash_accounts.clone(), nominee_account).await;
		validators_stashes.push((KeyPair::new(validator.signer().clone()), nominator_stash_accounts));
	}
...
async fn wait_for_successive_eras<C: ConnectionApi + WaitingExt + StakingApi>(
	...
	for _ in 0..eras_to_wait {
		info!("Era {} started, claiming rewards for era {}", current_era, current_era - 1);
		for (validator, nominators_stashes) in validators_and_nominator_stashes.iter() {
			let validator_connection = SignedConnection::new(address, KeyPair::new(validator.signer().clone())).await;
...
async fn nominate_validator(
	...
	for chunk in nominator_stash_accounts.clone().chunks(BOND_CALL_BATCH_LIMIT).map(|c| c.to_vec()) {
		let stake = (rng.gen::<Balance>() % 100) * TOKEN + MIN_NOMINATOR_BOND;
		root_connection.batch_bond(&chunk, stake, TxStatus::InBlock).await.unwrap();
	}
	...
	for chunks in nominator_nominee_accounts.chunks(NOMINATE_CALL_BATCH_LIMIT) {
		root_connection.batch_nominate(chunks, TxStatus::InBlock).await.unwrap();
	}
}
...
async fn bond_validators_funds_and_rotate_keys(address_ip_without_port: &str, validators: Vec<KeyPair>) {
	for (i, validator) in validators.into_iter().enumerate() {
		...
		connection.bond(MIN_VALIDATOR_BOND, TxStatus::InBlock).await.unwrap();
		rotate_validator_keys(&connection).await.unwrap();
	}
}
...
async fn send_validate_txs(address: &str, controllers: Vec<KeyPair>) {
	...
	for controller in controllers {
		...
		handles.push(tokio::spawn(async move {
			let connection = SignedConnection::new(&node_address, KeyPair::new(controller.signer().clone())).await;
...
async fn generate_nominator_accounts_with_minimal_bond<S: SignedConnectionApi>(
	...
	info!("Generating nominator accounts for validator {}", validator_number);
...
async fn payout_stakers_and_assert_locked_balance(
	...
	let locked_stash_balances_before_payout = stash_connection.locks(accounts_to_check_balance, None).await;
	stash_connection.payout_stakers(stash_account.clone(), era - 1, TxStatus::Finalized).await.unwrap();
	let locked_stash_balances_after_payout = stash_connection.locks(accounts_to_check_balance, None).await;
