...
This crate contains e2e test scenarios for the setheum-node.
...
where you can find your feature env address in https://github.com/Cardinal-Cryptography/setheum-node/deployments

In you have docker image of `e2e-client`, you can run above test with command
```bash
docker run --network host -e NODE_URL="wss://ws-fe-a0-29025887979136.dev.azero.dev:443" -e TEST_CASES="finalization::finalization" -e RUST_LOG=info  setheum-e2e-client:latest
```
