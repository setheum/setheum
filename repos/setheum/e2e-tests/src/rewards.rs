...
use setheum_client::{
	...
	utility::BlocksApi,
	waiting::{BlockStatus, SetheumWaiting, WaitingExt},
...
const COMMITTEE_SEATS: CommitteeSeats =
	CommitteeSeats { reserved_seats: 2, non_reserved_seats: 2, non_reserved_finality_seats: 2 };
...
pub async fn set_invalid_keys_for_validator<S: WaitingExt + SessionUserApi>(
	...
	for con in &controller_connections {
		...
		con.set_keys(invalid_keys.to_vec().into(), TxStatus::Finalized).await.unwrap();
	}
...
pub(super) async fn reset_validator_keys<S: AuthorRpc + WaitingExt + SessionUserApi>(
	...
	controller_connection.set_keys(validator_keys, TxStatus::InBlock).await.unwrap();

	// wait until our node is forced to use new keys, i.e. current session + 2
	controller_connection.wait_for_n_sessions(2, BlockStatus::Best).await;
...
pub async fn download_exposure<S: StakingApi>(
	...
	let exposure = connection.get_exposure(era, account_id, Some(beginning_of_session_block_hash)).await;
	info!("Validator {} has own exposure of {} and total of {}.", account_id, exposure.own, exposure.total);
...
fn check_rewards(
	...
	for (account, reward) in &validator_reward_points {
		...
		let retrieved_reward = *retrieved_reward_points.get(account).unwrap_or_else(|| {
			panic!("missing account={account} in retrieved collection of reward points {validator_reward_points:?}")
		});
		...
		info!("{} reward_ratio: {}; retrieved_ratio: {}.", account, reward_ratio, retrieved_ratio);
...
async fn get_node_performance<S: ElectionsApi + CommitteeManagementApi>(
	...
	info!("Validator {}, lenient performance {:?}.", account_id, lenient_performance);
	lenient_performance
}
...
pub async fn check_points<S: AsConnection + Sync>(
	...
	connection.wait_for_block(|n| n >= end_of_session_block, BlockStatus::Finalized).await;

	let beginning_of_session_block_hash = connection.get_block_hash(beginning_of_session_block).await?;
	let end_of_session_block_hash = connection.get_block_hash(end_of_session_block).await?;
	let before_end_of_session_block_hash = connection.get_block_hash(end_of_session_block - 1).await?;
	...
	info!("Blocks to produce per session: {} - session period {}.", blocks_to_produce_per_session, session_period);
	...
	let validator_reward_points_current_session: HashMap<AccountId, RewardPoint> = validator_reward_points_current_era
		.into_iter()
		.map(|(account_id, reward_points)| {
			let reward_points_previous_session =
				validator_reward_points_previous_session.get(&account_id).unwrap_or(&0);
	...
	let members_bench_uptime = members_bench.into_iter().map(|account_id| (account_id, 1.0));

	let mut reward_points: HashMap<_, _> = members_uptime.into_iter().chain(members_bench_uptime).collect();

	let members_count = reward_points.len() as f64;
	for (account_id, reward_points) in reward_points.iter_mut() {
		let exposure = download_exposure(connection, era, account_id, beginning_of_session_block_hash.unwrap()).await;
		*reward_points *= exposure as f64 / members_count;
	}

	check_rewards(reward_points, validator_reward_points_current_session, max_relative_difference)
}
...
pub async fn setup_validators(
	...
	let members: Vec<_> = get_validators_keys(config).iter().map(|kp| account_from_keypair(kp.signer())).collect();
	...
	let network_seats = root_connection.get_committee_seats(Some(first_block_in_session)).await;

	let era_validators =
		EraValidators { reserved: reserved_members.to_vec(), non_reserved: non_reserved_members.to_vec() };
	...
	let network_validators = root_connection.get_current_era_validators(first_block_in_session).await;
	...
	let network_seats = root_connection.get_committee_seats(first_block_in_session).await;
...
pub async fn validators_bond_extra_stakes(config: &Config, additional_stakes: &[Balance]) {
	...
	let accounts_keys: Vec<NodeKeys> = get_validators_seeds(config).into_iter().map(|seed| seed.into()).collect();
	for (account_keys, additional_stake) in accounts_keys.into_iter().zip(additional_stakes.iter()) {
		...
		stash_connection.bond_extra_stake(*additional_stake, TxStatus::Finalized).await.unwrap();
	}
}
