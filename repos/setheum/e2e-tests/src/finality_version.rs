...
pub async fn check_finality_version_at_block(
	...
	info!("Checking current session finality version for block {}", block_number);
	let block_hash = connection.get_block_hash(block_number).await.expect("Should have been able to get a block hash!");
...
pub async fn check_next_session_finality_version_at_block(
	...
	info!("Checking next session finality version for block {}", block_number);
	let block_hash = connection.get_block_hash(block_number).await.expect("Should have been able to get a block hash!");
