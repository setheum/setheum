...
use setheum_client::{
	...
	raw_keypair_from_string, AccountId, KeyPair, RawKeyPair, SignedConnection, SignedConnectionApi, TxStatus,
};
...
pub fn get_test_validators(config: &Config) -> EraValidators<KeyPair> {
	...
	let reserved = all_validators[0..2].iter().map(|k| KeyPair::new(k.clone())).collect();
	let non_reserved = all_validators[2..].iter().map(|k| KeyPair::new(k.clone())).collect();

	EraValidators { reserved, non_reserved }
}
...
pub fn setup_accounts(desired_validator_count: u32) -> Accounts {
	...
	let stash_keys: Vec<_> = stash_seeds.clone().map(|s| keypair_from_string(&s)).collect();
	let stash_raw_keys = stash_seeds.map(|s| raw_keypair_from_string(&s)).collect();
	let stash_accounts = stash_keys.iter().map(|k| account_from_keypair(k.signer())).collect();

	Accounts { stash_keys, stash_accounts, stash_raw_keys }
}
...
pub async fn prepare_validators<S: SignedConnectionApi + AuthorRpc>(
	...
	connection
		.batch_transfer_keep_alive(&accounts.stash_accounts, MIN_VALIDATOR_BOND + TOKEN, TxStatus::Finalized)
		.await
		.unwrap();
	...
	for (i, stash) in accounts.stash_raw_keys.iter().enumerate() {
		...
		handles.push(tokio::spawn(async move {
			connection.bond(MIN_VALIDATOR_BOND, TxStatus::Finalized).await.unwrap();
			let connection =
				SignedConnection::new(&validator_address((i + 1) as u32), KeyPair::new(stash.clone())).await;
			let keys = connection.author_rotate_keys().await.unwrap();
			connection.set_keys(keys, TxStatus::Finalized).await.unwrap();
