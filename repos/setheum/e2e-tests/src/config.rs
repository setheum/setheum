...
use setheum_client::{KeyPair, RootConnection, SignedConnection};
use url::Url;
...
static GLOBAL_CONFIG: Lazy<Config> = Lazy::new(|| {
	...
	let validators_seeds = env::var("VALIDATORS_SEEDS").ok().map(|s| s.split(',').map(|s| s.to_string()).collect());
...
where
	...
	T::Err: std::fmt::Debug,
{
	env::var(name).ok().map(|v| v.parse().unwrap_or_else(|_| panic!("Failed to parse env var {name}")))
}
...
impl NodeConfig {
	pub fn new(node: String, name: String, synthetic_network_url: Url, ip_address: Ipv4Addr, account: KeyPair) -> Self {
		Self { name, synthetic_network_url: synthetic_network_url.into(), ip_address, node, account }
	}
...
fn try_node_name_into_ip_address(node_name: String) -> anyhow::Result<Ipv4Addr> {
	...
	for addr in node_name
		.to_socket_addrs()
		.context(format!("Failed to convert node's name ({}) into IPv4 addrress.", node_name))?
	{
		if let std::net::SocketAddr::V4(socket_address) = addr {
			return Ok(*socket_address.ip());
		}
	}
	Err(anyhow::anyhow!("Unable to convert node's name ({}) into IPv4 address.", node_name))
}
...
impl Config {
	...
	pub fn validator_names(&self) -> Vec<String> {
		(1..self.validator_count + 1).map(|id| format!("Node{id}")).collect()
	}

	pub fn synthetic_network_urls(&self) -> Vec<String> {
		match &self.test_case_params.synthetic_network_urls {
			Some(urls) => urls.clone(),
			None => self.validator_names().into_iter().map(|node_name| format!("http://{node_name}:80/qos")).collect(),
		}
	}
	...
	pub fn nodes_configs(&self) -> anyhow::Result<std::vec::IntoIter<NodeConfig>> {
		...
		.map(|(node_name, synthetic_network_url, ip_address, account)| -> anyhow::Result<_> {
			Ok(NodeConfig::new(
