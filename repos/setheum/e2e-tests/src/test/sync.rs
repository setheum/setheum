...
use crate::{
	...
	synthetic_network::{
		await_finalized_blocks, await_new_blocks, execute_synthetic_network_test, ConnectivityConfiguration,
...
pub async fn one_node_catching_up_and_then_becoming_necessary_for_consensus() -> anyhow::Result<()> {
	...
	if config.validator_count < 4 {
		return Err(anyhow!("minimal required number of validators to run this test is 4"));
	}
	let nodes_configs: Vec<_> =
		config.nodes_configs().context("unable to build configuration for test nodes")?.collect();
	...
	let final_configuration = reconnect_configuration.merge(vec![left_side, right_side.clone()].into());
...
pub async fn into_two_groups_and_one_quorum_and_switch_quorum_between_them() -> anyhow::Result<()> {
	...
	if config.validator_count < 7 {
		return Err(anyhow!("minimal required number of validators to run this test is 7"));
	}

	let nodes_configs = config.nodes_configs().context("unable to build configuration for test nodes")?;

	let (left_side, right_side) = nodes_configs.as_slice().split_at((nodes_configs.len() - 1) / 3);

	let (left_side_after_reconnect, right_side_after_reconnect) =
		nodes_configs.as_slice().split_at(nodes_configs.len() - ((nodes_configs.len() - 1) / 3));

	let disconnect_configuration = NodesConnectivityConfiguration::from(vec![left_side.to_vec(), right_side.to_vec()]);
	let reconnect_configuration = disconnect_configuration.clone().reconnect();
...
pub async fn into_two_groups_one_with_quorum() -> anyhow::Result<()> {
	...
	let config = setup_test();
	if config.validator_count < 7 {
		return Err(anyhow!("provided test-network is to small ({0}), should be >= 7", config.validator_count,));
	}
...
pub async fn large_finalization_stall() -> anyhow::Result<()> {
	...
	let config = setup_test();
	if config.validator_count < 4 {
		return Err(anyhow!("provided test-network is to small ({0}), should be >= 4", config.validator_count,));
	}
	...
	let connections = join_all(
		nodes_configs
			.as_slice()
			.into_iter()
			.map(|config| async move { (config.node_name().to_owned(), config.create_signed_connection().await) }),
	)
	.await;
	...
	for node in nodes_configs.as_slice() {
		...
		node_configuration.disconnect_with_ports(
			nodes_configs
				.as_slice()
				.iter()
				.map(|node| (node.ip_address().clone(), PortRange::from(VALIDATOR_NETWORK_PORT))),
		);
	...
	execute_synthetic_network_test(nodes_configs.clone().as_slice(), async move {
		...
		for (node_name, connection) in connections.iter() {
			...
			let first_block_of_session = connection
				...
				.await?
				.context(format!("unable to retrieve first block of session {session} at node {node_name}"))
				.unwrap();
			let first_block_of_session = connection
		...
		for (node_name, connection) in connections.iter() {
			...
			connection
				.wait_for_block(|block| block >= wait_block, setheum_client::waiting::BlockStatus::Best)
				.await;
		}

		reconnect_configuration.commit().await?;

		await_finalized_blocks(nodes_configs.as_slice(), wait_block, NUMBER_OF_BLOCKS_TO_WAIT_AFTER_RECONNECT).await
	})
	.await
}
...
async fn perform_test(
	...
	execute_synthetic_network_test(all_nodes_to_check, async move {
		...
		await_finalized_blocks(nodes_to_check_after_reconfigure, best_seen_block, blocks_to_wait_after_reconnect).await
	})
	.await
}
