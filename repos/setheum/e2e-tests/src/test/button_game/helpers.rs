...
use super::contracts::{ButtonInstance, MarketplaceInstance, PSP22TokenInstance, SimpleDexInstance, WAzeroInstance};
...
pub(super) async fn ticket_token<C: ConnectionApi>(
	...
	PSP22TokenInstance::new(button.ticket_token(conn).await?, &config.test_case_params.ticket_token_metadata)
}
...
pub(super) async fn reward_token<C: ConnectionApi>(
	...
	PSP22TokenInstance::new(button.reward_token(conn).await?, &config.test_case_params.reward_token_metadata)
}
...
pub(super) async fn marketplace<C: ConnectionApi>(
	...
	MarketplaceInstance::new(button.marketplace(conn).await?, &config.test_case_params.marketplace_metadata)
}

/// Derives a test account based on a randomized string
pub fn random_account() -> KeyPair {
	setheum_client::keypair_from_string(&format!("//TestAccount/{}", rand::thread_rng().gen::<u128>()))
}

/// Transfer `amount` from `from` to `to`
pub async fn transfer(conn: &Connection, from: &KeyPair, to: &KeyPair, amount: Balance) -> Result<()> {
...
pub(super) async fn setup_wrapped_azero_test(config: &Config) -> Result<WAzeroTestContext> {
	...
	tokio::spawn(async move {
		...
		listen_contract_events(&listen_conn, &contract_metadata, events_tx).await.unwrap();
	});

	let events = BufferedReceiver::new(events_rx, Duration::from_secs(10));

	Ok(WAzeroTestContext { conn, account, wazero, events })
}
...
pub(super) async fn setup_dex_test(config: &Config) -> Result<DexTestContext> {
	...
	let token1 = reward_token_for_button(config, &conn, &config.test_case_params.early_bird_special).await?;
	let token2 = reward_token_for_button(config, &conn, &config.test_case_params.the_pressiah_cometh).await?;
	let token3 = reward_token_for_button(config, &conn, &config.test_case_params.back_to_the_future).await?;
	...
	tokio::spawn(async move {
		let contract_metadata = vec![c1.as_ref().into(), c2.as_ref().into(), c3.as_ref().into(), c4.as_ref().into()];
	...
	Ok(DexTestContext { conn, authority, account, dex, token1, token2, token3, events })
}
...
pub(super) async fn setup_button_test(
	...
	Ok(ButtonTestContext { button, ticket_token, reward_token, marketplace, conn, events, authority, player })
}
...
async fn basic_test_context(config: &Config) -> Result<(Connection, KeyPair, KeyPair)> {
	...
	let authority = setheum_client::keypair_from_string(&config.sudo_seed);
	let account = random_account();

	info!("Transferring 100 SETHEUM to a random test account");
	transfer(&conn, &authority, &account, setheums(100)).await?;
...
impl<T> BufferedReceiver<T> {
	pub fn new(receiver: UnboundedReceiver<T>, default_timeout: Duration) -> Self {
		Self { buffer: Vec::new(), receiver, default_timeout }
	}
...
pub(super) async fn wait_for_death<C: ConnectionApi>(conn: &C, button: &ButtonInstance) -> Result<()> {
...
pub async fn assert_recv_id(events: &mut BufferedReceiver<Result<ContractEvent>>, id: &str) -> ContractEvent {
	assert_recv(events, |event| event.name == Some(id.to_string()), &format!("Expected {id:?} contract event")).await
}
...
pub async fn refute_recv_id(events: &mut BufferedReceiver<Result<ContractEvent>>, id: &str) {
	if let Ok(event) = recv_timeout_with_log(events, |event| event.name == Some(id.to_string())).await {
