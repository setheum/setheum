...
use setheum_client::{contract_transcode::Value, pallets::system::SystemApi};
use tokio::time::sleep;

use crate::{
	config::{setup_test, Config},
	test::button_game::helpers::{
		assert_recv_id, mega, refute_recv_id, setheums, setup_button_test, setup_dex_test, setup_wrapped_azero_test,
		wait_for_death, ButtonTestContext, DexTestContext, WAzeroTestContext,
	},
};
...
pub async fn wrapped_azero() -> Result<()> {
	...
	let WAzeroTestContext { conn, account, wazero, mut events, .. } = setup_wrapped_azero_test(config).await?;
	let account_conn = &sign(&conn, &account);
	let account_id = account.account_id();

	wazero.wrap(account_conn, setheums(2)).await?;
	...
	assert!(event.data.get("amount") == Some(&Value::UInt(setheums(2))));
	assert!(wazero.balance_of(account_conn, account.account_id()).await? == setheums(2));

	let balance_before = conn.get_free_balance(account_id.clone(), None).await;
	wazero.unwrap(account_conn, setheums(1)).await?;

	let event = assert_recv_id(&mut events, "Unwrapped").await;
	let balance_after = conn.get_free_balance(account_id.clone(), None).await;
	let max_fee = setheums(1) / 100;
	assert!(balance_after - balance_before > setheums(1) - max_fee);
	let_assert!(Some(Value::Literal(acc_id)) = event.data.get("caller"));
	assert!(*acc_id == account_id.to_string());
	assert!(event.data.get("amount") == Some(&Value::UInt(setheums(1))));
	assert!(wazero.balance_of(account_conn, account.account_id()).await? == setheums(1));
...
pub async fn simple_dex() -> Result<()> {
	let config = setup_test();
	let DexTestContext { conn, authority, account, dex, token1, token2, token3, mut events } =
	...
	dex.add_swap_pair(authority_conn, token1.into(), token2.into()).await?;
	assert_recv_id(&mut events, "SwapPairAdded").await;

	dex.add_swap_pair(authority_conn, token2.into(), token1.into()).await?;
	assert_recv_id(&mut events, "SwapPairAdded").await;

	dex.add_swap_pair(authority_conn, token1.into(), token3.into()).await?;
	assert_recv_id(&mut events, "SwapPairAdded").await;

	token1.mint(authority_conn, authority.account_id(), mega(3000)).await?;
	assert_recv_id(&mut events, "Transfer").await;
	token2.mint(authority_conn, authority.account_id(), mega(5000)).await?;
	assert_recv_id(&mut events, "Transfer").await;
	token3.mint(authority_conn, authority.account_id(), mega(10000)).await?;
	assert_recv_id(&mut events, "Transfer").await;

	token1.approve(authority_conn, &dex.into(), mega(3000)).await?;
	assert_recv_id(&mut events, "Approval").await;
	token2.approve(authority_conn, &dex.into(), mega(5000)).await?;
	assert_recv_id(&mut events, "Approval").await;
	token3.approve(authority_conn, &dex.into(), mega(10000)).await?;
	assert_recv_id(&mut events, "Approval").await;

	token1.transfer(authority_conn, dex.contract.address(), mega(3000)).await?;

	assert_recv_id(&mut events, "Transfer").await;

	token2.transfer(authority_conn, dex.contract.address(), mega(5000)).await?;

	assert_recv_id(&mut events, "Transfer").await;

	token3.transfer(authority_conn, dex.contract.address(), mega(10000)).await?;

	assert_recv_id(&mut events, "Transfer").await;

	let more_than_liquidity = mega(1_000_000);
	let res = dex.swap(account_conn, token1, 100, token2, more_than_liquidity).await;
	assert!(res.is_err());
	refute_recv_id(&mut events, "Swapped").await;

	let initial_amount = mega(100);
	token1.mint(authority_conn, account.account_id(), initial_amount).await?;
	let expected_output = dex.out_given_in(account_conn, token1, token2, initial_amount).await?;
	assert!(expected_output > 0);

	let at_most_10_percent_slippage = expected_output * 9 / 10;
	token1.approve(account_conn, &dex.into(), initial_amount).await?;
	dex.swap(account_conn, token1, initial_amount, token2, at_most_10_percent_slippage).await?;
	assert_recv_id(&mut events, "Swapped").await;
	assert!(token2.balance_of(&conn, account.account_id()).await? == expected_output);

	token2.approve(account_conn, &dex.into(), expected_output).await?;
	dex.swap(account_conn, token2, expected_output, token1, mega(90)).await?;
	assert_recv_id(&mut events, "Swapped").await;

	let balance_after = token1.balance_of(&conn, account.account_id()).await?;
	assert!(initial_amount.abs_diff(balance_after) <= 1);
	assert!(dex.out_given_in(account_conn, token1, token2, initial_amount).await?.abs_diff(expected_output) <= 1);

	token1.approve(account_conn, &dex.into(), balance_after).await?;
	let unreasonable_slippage = expected_output * 11 / 10;
	assert!(
		dex.swap(account_conn, token1, balance_after, token2, unreasonable_slippage,).await.is_err(),
		"expected swap to fail"
	);
	refute_recv_id(&mut events, "Swapped").await;

	dex.swap(account_conn, token1, balance_after, token3, mega(90)).await?;
	assert_recv_id(&mut events, "Swapped").await;

	// can't swap a pair not on the whitelist
	dex.remove_swap_pair(authority_conn, token3.into(), token1.into()).await?;
	assert_recv_id(&mut events, "SwapPairRemoved").await;

	let balance_token3 = token3.balance_of(&conn, account.account_id()).await?;
	token3.approve(account_conn, &dex.into(), balance_token3).await?;
	assert!(
		dex.swap(account_conn, token3, balance_token3, token1, mega(90)).await.is_err(),
...
pub async fn marketplace() -> Result<()> {
	let config = setup_test();
	let ButtonTestContext { conn, authority, player, marketplace, ticket_token, reward_token, mut events, .. } =
	...
	ticket_token.transfer(&sign(&conn, &authority), &marketplace.as_ref().into(), 2).await?;
	...
	reward_token.mint(&sign(&conn, &authority), player.account_id(), player_balance).await?;
	reward_token.approve(&sign(&conn, player), &marketplace.as_ref().into(), later_price).await?;
	...
	assert!(
		marketplace.buy(&sign(&conn, player), Some(latest_price / 2)).await.is_err(),
	...
	marketplace.buy(&sign(&conn, player), Some(latest_price * 2)).await?;
...
pub async fn button_game_reset() -> Result<()> {
	let config = setup_test();
	let ButtonTestContext { conn, button, mut events, authority, marketplace, ticket_token, .. } =
		setup_button_test(config, &config.test_case_params.early_bird_special).await?;

	let round_old = button.round(&conn).await?;
	let deadline_old = button.deadline(&conn).await?;
	let marketplace_initial = ticket_token.balance_of(&conn, &marketplace.as_ref().into()).await?;
	ticket_token.transfer(&sign(&conn, &authority), &button.as_ref().into(), 1).await?;
	let button_balance = ticket_token.balance_of(&conn, &button.as_ref().into()).await?;
	...
	assert!(
		ticket_token.balance_of(&conn, &marketplace.as_ref().into()).await? == marketplace_initial + button_balance
	);
...
pub async fn back_to_the_future() -> Result<()> {
	...
	button_game_play(
		...
		|first_presser_time, first_presser_score, second_presser_time, second_presser_score| {
			assert!(first_presser_time.cmp(&second_presser_time) == first_presser_score.cmp(&second_presser_score));
		},
	)
	.await
}
...
async fn button_game_play<F: Fn(u128, u128, u128, u128)>(
	...
	let ButtonTestContext { conn, button, mut events, authority, ticket_token, reward_token, player, .. } =
		setup_button_test(config, button_contract_address).await?;
	info!("Setup done");

	let player = &player;

	ticket_token.transfer(&sign(&conn, &authority), player.account_id(), 2).await?;
	...
	let old_button_balance = ticket_token.balance_of(&conn, &button.as_ref().into()).await?;

	ticket_token.approve(&sign(&conn, player), &button.as_ref().into(), 2).await?;
	...
	assert!(ticket_token.balance_of(&conn, player.account_id()).await? == 1);
	assert!(ticket_token.balance_of(&conn, &button.as_ref().into()).await? == old_button_balance + 1);
	...
	score_check(first_presser_time.into(), first_reward_amount, second_presser_time.into(), second_reward_amount);
	...
	assert!(reward_token.balance_of(&conn, player.account_id()).await? == player_rewards + pressiah_reward);
