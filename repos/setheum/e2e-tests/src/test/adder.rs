...
/// This test exercises the setheum-client code for interacting with contracts by testing a simple
...
pub async fn adder_events_listening() -> Result<()> {
	...
	let contract =
		Arc::new(AdderInstance::new(&config.test_case_params.adder, &config.test_case_params.adder_metadata)?);
	...
	contract.set_name(&account.sign(&conn), Some(new_name)).await?;
...
pub async fn adder_fetching_events() -> Result<()> {
	...
	let contract = AdderInstance::new(&config.test_case_params.adder, &config.test_case_params.adder_metadata)?;
...
/// This test ensures that `setheum-client` won't submit call if dry-run fails.
#[tokio::test]
pub async fn adder_dry_run_failure() -> Result<()> {
	...
	let caller_balance_before = conn.get_free_balance(account.account_id().clone(), None).await;

	// Should fail due to the overflow check in contract.
	let result = contract.add(&account.sign(&conn), u32::MAX).await;
	assert!(result.is_err());

	let caller_balance_after = conn.get_free_balance(account.account_id().clone(), None).await;
...
pub async fn adder_readonly_calls() -> Result<()> {
	...
	let block_with_state_0 = conn.get_block_hash(conn.get_best_block().await?.unwrap()).await.unwrap().unwrap();
...
pub async fn adder_setting_gas_limits() -> Result<()> {
	...
	let dry_run_result = contract
		.contract
		.exec_dry_run(&conn, account.account_id().clone(), "add", &["1"], Default::default())
		.await?;
	...
	assert!(contract
		.add_with_params(
			&account.sign(&conn),
			1,
			ExecCallParams::new().gas_limit(Weight::new(gas_required.ref_time() - 1, gas_required.proof_size()))
		)
		.await
		.is_err());
	assert!(contract
		.add_with_params(
			&account.sign(&conn),
			1,
			ExecCallParams::new().gas_limit(Weight::new(gas_required.ref_time(), gas_required.proof_size() - 1))
		)
		.await
		.is_err());
	assert!(contract
		.add_with_params(
			&account.sign(&conn),
			1,
			ExecCallParams::new().gas_limit(Weight::new(gas_required.ref_time(), gas_required.proof_size()))
		)
		.await
		.is_ok());
	Ok(())
...
impl AdderInstance {
	pub fn new(address: &Option<String>, metadata_path: &Option<String>) -> Result<Self> {
		...
		let metadata_path = metadata_path.as_ref().context("Adder contract metadata not set")?;

		let address =
			AccountId::from_str(address).ok().with_context(|| format!("Failed to parse address: {address}"))?;
	...
	pub async fn get_at<C: ConnectionApi>(&self, conn: &C, at: BlockHash) -> Result<u32> {
		self.contract.read0(conn, "get", ReadonlyCallParams::new().at(at)).await
	}
	...
	pub async fn add_with_params<S: SignedConnectionApi>(
		...
		self.contract.exec(conn, "add", &[value.to_string()], params).await
	}

	pub async fn set_name<S: SignedConnectionApi>(&self, conn: &S, name: Option<&str>) -> Result<TxInfo> {
		...
		self.contract.exec(conn, "set_name", &[name], Default::default()).await
	}

	pub async fn get_name<C: ConnectionApi>(&self, conn: &C) -> Result<Option<String>> {
		let res: Option<String> = self.contract.read0(conn, "get_name", Default::default()).await?;
