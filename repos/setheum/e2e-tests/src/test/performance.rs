...
use log::info;
use primitives::{
	SessionCount, DEFAULT_FINALITY_BAN_MINIMAL_EXPECTED_PERFORMANCE, DEFAULT_FINALITY_BAN_SESSION_COUNT_THRESHOLD,
};
use setheum_client::pallets::committee_management::CommitteeManagementSudoApi;
use setheum_client::{
	pallets::{
		committee_management::CommitteeManagementApi,
		elections::{ElectionsApi, ElectionsSudoApi},
		session::SessionApi,
		setheum::{SetheumApi, SetheumSudoApi},
	},
...
const SBFT_PERFORMANCE_VERSION: u32 = 5;

#[tokio::test]
async fn all_validators_have_ideal_performance() -> anyhow::Result<()> {
	let config = crate::config::setup_test();
	let (root_connection, reserved_validators, non_reserved_validators, _) = setup_test(config).await?;
	let all_validators = reserved_validators.iter().chain(non_reserved_validators.iter());

	set_finality_version(SBFT_PERFORMANCE_VERSION, &root_connection).await?;
	...
	root_connection.wait_for_n_sessions(1, BlockStatus::Best).await;

	let block = root_connection.last_block_of_session(session_id).await?;
	let scores = root_connection.sbft_scores(session_id, block).await.unwrap();
...
async fn one_validator_is_dead() -> anyhow::Result<()> {
	...
	assert_eq!(ban_config.underperformed_session_count_threshold, production_underperformed_threshold);
	check_finality_ban_config(
		&root_connection,
		DEFAULT_FINALITY_BAN_MINIMAL_EXPECTED_PERFORMANCE,
		DEFAULT_FINALITY_BAN_SESSION_COUNT_THRESHOLD,
	...
	root_connection.wait_for_session(score_session_id + 1, BlockStatus::Best).await;

	let scores = root_connection.sbft_scores(score_session_id, None).await.unwrap();
...
async fn set_finality_version(finality_version: u32, root_connection: &RootConnection) -> anyhow::Result<()> {
	...
	if current_finality_version < SBFT_PERFORMANCE_VERSION {
...
async fn setup_test(
	config: &Config,
) -> anyhow::Result<(RootConnection, Vec<AccountId>, Vec<AccountId>, CommitteeSeats)> {
	...
	let non_reserved_validators =
		account_ids_from_keys(&validator_keys.non_reserved).into_iter().take(2).collect::<Vec<_>>();
	...
	if send_change_validators_xt {
		info!("Changing validators {:?}.", (&maybe_new_reserved, &maybe_new_non_reserved, &maybe_new_committee_seats));
		root_connection
			.change_validators(maybe_new_reserved, maybe_new_non_reserved, maybe_new_committee_seats, TxStatus::InBlock)
			.await?;
	...
	Ok((root_connection, reserved_validators, non_reserved_validators, seats))
}
...
fn check_validators(
	...
	assert_eq!(HashSet::<_>::from_iter(&era_validators.reserved), HashSet::<_>::from_iter(expected_reserved));
	assert_eq!(HashSet::<_>::from_iter(&era_validators.non_reserved), HashSet::<_>::from_iter(expected_non_reserved));

	era_validators
}

async fn change_finality_version<C: SessionApi + SetheumSudoApi + SetheumWaiting>(
	...
	connection
		.schedule_finality_version_change(finality_version, session_for_upgrade, TxStatus::InBlock)
		.await?;
	connection.wait_for_session(session_for_upgrade, BlockStatus::Best).await;
...
async fn check_finality_ban_config<C: CommitteeManagementApi>(
	...
	assert_eq!(ban_config.minimal_expected_performance, expected_minimal_expected_performance);
	assert_eq!(ban_config.underperformed_session_count_threshold, expected_session_count_threshold);
}
...
async fn check_underperformed_validator_session_count<C: CommitteeManagementApi>(
	...
	assert_eq!(underperformed_validator_session_count, expected_session_count, "{}", validator);
}
