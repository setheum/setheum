...
use setheum_client::{
	api::{
		elections::events::ChangeValidators, runtime_types::sp_runtime::ModuleError, sudo::events::Sudid, DispatchError,
	},
...
pub async fn change_validators() -> anyhow::Result<()> {
	...
	connection
		.change_validators(
			...
			Some(CommitteeSeats { reserved_seats: 2, non_reserved_seats: 3, non_reserved_finality_seats: 2 }),
			TxStatus::InBlock,
		)
		.await?;
	...
	connection
		.wait_for_event(
			|e: &ChangeValidators| {
				info!(
					"[+] NewValidatorsEvent: reserved: {:#?}, non_reserved: {:#?}, committee_size: {:#?}",
					e.0, e.1, e.2
				);
				...
				e.0.iter().cloned().map(|x| x.0).collect::<Vec<_>>() == new_validators[0..2]
					...
					&& e.2
						== CommitteeSeats { reserved_seats: 2, non_reserved_seats: 3, non_reserved_finality_seats: 2 }
			},
			BlockStatus::Best,
		)
		.await;
	...
	let block_number = connection.get_best_block().await?.ok_or(anyhow!("Failed to retrieve best block!"))?;
...
pub async fn fail_changing_validators() -> anyhow::Result<()> {
	...
	connection
		.change_validators(
			Some(new_validators[0..2].to_vec()),
			Some(new_validators[2..].to_vec()),
			Some(CommitteeSeats { reserved_seats: 2, non_reserved_seats: 3, non_reserved_finality_seats: 4 }),
			TxStatus::InBlock,
		)
		.await?;
