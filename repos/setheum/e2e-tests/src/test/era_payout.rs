...
use primitives::{EraIndex, DEFAULT_SESSIONS_PER_ERA, DEFAULT_SESSION_PERIOD, MILLISECS_PER_BLOCK, TOKEN};
use setheum_client::{
	pallets::{
...
async fn get_era_duration<C: TimestampApi + BlocksApi>(era: EraIndex, connection: &C) -> u64 {
	...
	let current_era_first_block_hash = connection.get_block_hash(current_era_first_block).await.unwrap();
	let next_era_first_block_hash = connection.get_block_hash(next_era_first_block).await.unwrap();

	return connection.get_timestamp(next_era_first_block_hash).await.unwrap()
		- connection.get_timestamp(current_era_first_block_hash).await.unwrap();
}
...
async fn force_era_payout(config: &Config) -> anyhow::Result<()> {
	...
	root_connection.wait_for_session(starting_session + 1, BlockStatus::Best).await;

	// new era will start in the session `starting_session + 3`
	root_connection.force_new_era(TxStatus::InBlock).await?;
	root_connection.wait_for_session(starting_session + 3, BlockStatus::Best).await;
...
async fn normal_era_payout(config: &Config) -> anyhow::Result<()> {
	...
	let payout = root_connection.get_payout_for_era(active_era - 1, None).await;

	let actual_duration = get_era_duration(active_era - 1, &root_connection).await;

	let expected_era_duration = (DEFAULT_SESSIONS_PER_ERA * DEFAULT_SESSION_PERIOD) as u64 * MILLISECS_PER_BLOCK;
...
fn assert_within_delta_interval<T>(expected: T, actual: T, delta: T, quantity_name: &str, extra_msg_on_fail: &str)
where
	T: std::fmt::Display + std::ops::Add<Output = T> + std::ops::Sub<Output = T> + PartialOrd + Copy,
{
