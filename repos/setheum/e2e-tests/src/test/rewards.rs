...
pub async fn points_basic() -> anyhow::Result<()> {
	...
	info!("Checking rewards for sessions {}..{}.", start_session, end_session);
	...
	for session in start_session..end_session {
		...
		let (members_active, members_bench) =
			get_and_test_members_for_session(&connection, committee_size.clone(), &era_validators, session).await?;

		check_points(&connection, session, era, members_active, members_bench, members_per_session, MAX_DIFFERENCE)
			.await?
	}
...
pub async fn points_stake_change() -> anyhow::Result<()> {
	...
	validators_bond_extra_stakes(
		config,
		&[8 * MIN_VALIDATOR_BOND, 6 * MIN_VALIDATOR_BOND, 4 * MIN_VALIDATOR_BOND, 2 * MIN_VALIDATOR_BOND, 0],
	)
	.await;
...
pub async fn disable_node() -> anyhow::Result<()> {
	...
	let controller_connection = SignedConnection::new(&config.node, config.node_keys().validator).await;
	...
	for session in start_session..end_session {
		...
		let (members_active, members_bench) =
			get_and_test_members_for_session(&controller_connection, committee_size.clone(), &era_validators, session)
				.await?;
...
pub async fn force_new_era() -> anyhow::Result<()> {
	...
	connection.wait_for_session(start_session + 2, BlockStatus::Finalized).await;
	let active_era = connection.get_active_era(None).await;
	let current_session = connection.get_session(None).await;
	info!("After ForceNewEra | era: {}, session: {}", active_era, current_session);
...
pub async fn change_stake_and_force_new_era() -> anyhow::Result<()> {
	...
	validators_bond_extra_stakes(
		config,
		&[7 * MIN_VALIDATOR_BOND, 2 * MIN_VALIDATOR_BOND, 11 * MIN_VALIDATOR_BOND, 0, 4 * MIN_VALIDATOR_BOND],
	)
	.await;
...
async fn check_points_after_force_new_era<S: AsConnection + Sync>(
	...
	for idx in 0..3 {
		...
		info!("Testing points | era: {}, session: {}", era_to_check, session_to_check);

		let (members_active, members_bench) =
			get_and_test_members_for_session(connection, seats.clone(), era_validators, session_to_check).await?;
