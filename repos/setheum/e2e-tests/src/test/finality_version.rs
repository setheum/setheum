...
use setheum_client::{
	pallets::{committee_management::CommitteeManagementApi, session::SessionApi, setheum::SetheumSudoApi},
	utility::BlocksApi,
...
use crate::{
	config::setup_test,
	finality_version::{check_finality_version_at_block, check_next_session_finality_version_at_block},
};
...
pub async fn schedule_version_change() -> anyhow::Result<()> {
	...
	let version_for_upgrade = test_case_params.upgrade_to_version.unwrap_or(UPGRADE_TO_VERSION);
	let session_for_upgrade = current_session + test_case_params.upgrade_session.unwrap_or(UPGRADE_SESSION);
	let wait_sessions_after_upgrade =
		test_case_params.upgrade_finalization_wait_sessions.unwrap_or(UPGRADE_FINALIZATION_WAIT_SESSIONS);
	let session_after_upgrade = session_for_upgrade + wait_sessions_after_upgrade;

	connection
		.schedule_finality_version_change(version_for_upgrade, session_for_upgrade, TxStatus::Finalized)
		.await?;
	connection.wait_for_session(session_after_upgrade + 1, BlockStatus::Finalized).await;

	let block_number = connection.get_best_block().await?.ok_or(anyhow!("Failed to retrieve best block number!"))?;
	connection.wait_for_block(|n| n >= block_number, BlockStatus::Finalized).await;
...
pub async fn schedule_doomed_version_change_and_verify_finalization_stopped() -> anyhow::Result<()> {
	...
	connection
		.schedule_finality_version_change(version_for_upgrade, session_for_upgrade, TxStatus::Finalized)
		.await?;
	connection.wait_for_session(session_after_upgrade + 1, BlockStatus::Best).await;
	...
	let finalized_block = match finalized_block {
		Some(block) => block,
		_ => return Err(anyhow::Error::msg("somehow no block was finalized (even though we saw one)")),
	};
...
pub async fn finality_version_change() -> anyhow::Result<()> {
	...
	let session_with_finality_version_change = current_session + SESSIONS_AHEAD_FOR_FINALITY_VERSION_CHANGE;

	let start_point_in_sessions = session_with_finality_version_change as f64 + SCHEDULING_OFFSET_SESSIONS;
	...
	info!("Finality version check | start block: {} | end block: {}", CHECK_START_BLOCK, end_block,);
	...
	root_connection.wait_for_block(|n| n >= scheduling_block, BlockStatus::Finalized).await;
	...
	root_connection
		...
		.await?;
	root_connection.wait_for_block(|n| n >= end_block, BlockStatus::Finalized).await;

	let finality_change_block = session_with_finality_version_change * session_period;
	let last_block_with_default_next_session_finality_version = finality_change_block - session_period - 1;
	...
	for block in CHECK_START_BLOCK..(last_block_with_default_next_session_finality_version + 1) {
		check_finality_version_at_block(root_connection.as_connection(), block, LEGACY_FINALITY_VERSION as Version)
			.await;
	...
	for block in finality_change_block..(end_block + 1) {
		check_finality_version_at_block(root_connection.as_connection(), block, first_incoming_finality_version).await;
		check_next_session_finality_version_at_block(
			root_connection.as_connection(),
			block,
			first_incoming_finality_version,
