...
async fn setup_proxy(connection: RootConnection, proxy_type: ProxyType) -> ProxyHandle {
	...
	connection
		.as_signed()
		.transfer_keep_alive(account.account_id().clone(), 10_000 * TOKEN, TxStatus::Finalized)
		.await
		.expect("transfer should succeed");
	connection
		.as_signed()
		.transfer_keep_alive(proxy.account_id().clone(), 1000 * TOKEN, TxStatus::Finalized)
		.await
		.expect("transfer should succeed");

	let account_connection = SignedConnection::from_connection(connection.as_connection().clone(), account.clone());

	account_connection
		.add_proxy(proxy.account_id().clone(), proxy_type, 0, TxStatus::Finalized)
		.await
...
async fn perform_and_check_calls(
	...
	let proxy_connection = SignedConnection::from_connection(connection.clone(), handle.proxy.clone());

	for (call, status) in calls {
		let tx_info = proxy_connection
			.proxy(handle.account.account_id().clone(), call.clone(), TxStatus::InBlock)
			.await
			.unwrap();
...
pub async fn any_proxy_works() -> anyhow::Result<()> {
	...
	let calls = vec![
		(
			RuntimeCall::Balances(transfer_allow_death { dest: MultiAddress::Id(Static(test_id.clone())), value: 100 }),
			Success,
		),
		(
		...
		(RuntimeCall::Staking(bond { value: 1000, payee: RewardDestination::Staked }), Success),
		(RuntimeCall::Session(set_keys { keys: vec![0u8; 64].into(), proof: vec![] }), Success),
	];
...
pub async fn nomination_proxy_works() -> anyhow::Result<()> {
	...
	let test_account_connection =
		SignedConnection::from_connection(root_connection.as_connection().clone(), handle.account.clone());
	let validator_0_id = accounts_seeds_to_keys(&get_validators_seeds(config))[0].account_id().clone();

	let calls = vec![
		(RuntimeCall::Staking(bond { value: 1000, payee: RewardDestination::Staked }), Failure),
		(
			RuntimeCall::Utility(batch {
				calls: vec![RuntimeCall::Staking(bond { value: 1000, payee: RewardDestination::Staked })],
			}),
			Failure,
		),
	];
	...
	test_account_connection.bond(2500 * TOKEN, TxStatus::Finalized).await?;
	let calls = vec![
		(RuntimeCall::Staking(nominate { targets: vec![MultiAddress::Id(Static(validator_0_id.clone()))] }), Success),
