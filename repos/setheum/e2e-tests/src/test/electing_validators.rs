...
async fn assert_validators_are_elected_stakers<C: StakingRawApi>(
	...
	let expected_validators_as_keys: Vec<_> = connection
		...
		.map(|k| k.0)
		.collect();
	assert_eq!(expected_validators_as_keys_legacy.len(), expected_validators_as_keys.len());
	let stakers_legacy: Vec<_> =
		connection.get_stakers_storage_keys(current_era, None).await?.into_iter().map(|key| key.0).collect();
	let stakers: Vec<_> = connection
		...
		.map(|key| key.0)
		.collect();
	for (key_legacy, key) in expected_validators_as_keys_legacy.iter().zip(expected_validators_as_keys.iter()) {
...
fn min_num_sessions_to_see_all_non_reserved_validators(non_reserved_count: u32, non_reserved_seats: u32) -> u32 {
...
fn assert_enough_validators_left_after_chilling(
	...
	assert!(reserved_count >= reserved_to_chill_count, "Cannot have less than 0 reserved validators!");
	assert!(non_reserved_count >= non_reserved_to_chill_count, "Cannot have less than 0 non-reserved validators!");
...
pub async fn authorities_are_staking() -> anyhow::Result<()> {
	...
	let chilling_non_reserved = KeyPair::new(accounts.get_stash_raw_keys()[reserved_seats as usize].clone()); // first non-reserved validator

	let reserved_count = reserved_validators.len() as u32;
	let non_reserved_count = non_reserved_validators.len() as u32;

	assert_eq!(reserved_seats, reserved_count, "Desired {reserved_seats} reserved seats, got {reserved_count}!");
	assert_eq!(
	...
	assert_validators_are_elected_stakers(&connection, current_era, accounts.get_stash_accounts()).await?;

	let min_num_sessions = min_num_sessions_to_see_all_non_reserved_validators(non_reserved_count, non_reserved_seats);
	...
	connection.wait_for_n_eras(2, BlockStatus::Best).await;
	let current_era = connection.get_current_era(None).await;
	info!("Subset of validators should be in force (era: {})", current_era);
