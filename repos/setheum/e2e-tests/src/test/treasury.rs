...
async fn balance_info<C: ConnectionApi + AsConnection>(connection: &C) -> (Balance, Balance) {
	let treasury_balance = connection.get_free_balance(connection.treasury_account().await, None).await;
	let issuance = connection.total_issuance(None).await;
	info!("[+] Treasury balance: {}. Total issuance: {}.", treasury_balance, issuance);
...
pub async fn channeling_fee_and_tip() -> anyhow::Result<()> {
	...
	connection.wait_for_era(active_era + 1, BlockStatus::Best).await;

	let (treasury_balance_before, issuance_before) = balance_info(&connection).await;

	let transfer = connection.transfer_keep_alive_with_tip(to, transfer_amount, tip, TxStatus::Finalized).await?;
...
fn check_issuance(issuance_before: Balance, issuance_after: Balance) {
	assert!(issuance_after >= issuance_before, "Unexpectedly {} was burned", issuance_before - issuance_after,);
	let minted = issuance_after - issuance_before;
	assert_eq!(issuance_before, issuance_after, "Unexpectedly {minted} was minted");
}

fn check_treasury_balance(treasury_balance_before: Balance, treasury_balance_after: Balance, fee: Balance) {
...
pub async fn treasury_access() -> anyhow::Result<()> {
	...
	assert_eq!(proposals_counter_before + 2, proposals_counter_after, "Proposal has not been created");
...
async fn reject_treasury_proposal(connection: &RootConnection, id: u32) -> anyhow::Result<()> {
	...
	let handle = tokio::spawn(async move {
		handle_connection
			.wait_for_event(|e: &Rejected| e.proposal_index == id, BlockStatus::Finalized)
			.await;
	});
