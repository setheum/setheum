...
pub async fn validators_rotate() -> anyhow::Result<()> {
	...
	root_connection.wait_for_n_sessions(TEST_LENGTH, BlockStatus::Finalized).await;

	let mut non_reserved_count = HashMap::new();

	for session in current_session..current_session + TEST_LENGTH {
		let elected = connection.get_validators(connection.first_block_of_session(session).await?).await;
		...
		let reserved_included = reserved_validators.clone().iter().all(|reserved| elected.contains(reserved));

		let non_reserved_include = non_reserved.iter().all(|non_reserved| elected.contains(non_reserved));

		let only_expected_validators = elected
			.iter()
			.all(|elected| reserved_validators.contains(elected) || non_reserved.contains(elected));

		assert!(reserved_included, "Reserved nodes should always be present, session #{session}");
		assert!(non_reserved_include, "Missing non reserved node, session #{session}");
		assert!(only_expected_validators, "Only expected validators should be present, session #{session}");
	}
