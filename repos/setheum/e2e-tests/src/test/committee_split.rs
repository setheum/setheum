...
async fn prepare_test() -> anyhow::Result<()> {
	...
	let seats = CommitteeSeats { reserved_seats: 3, non_reserved_seats: 4, non_reserved_finality_seats: 1 };
...
async fn split_disable(validators: &[u32]) -> anyhow::Result<()> {
	...
	let block_before = connection.get_best_block().await?.expect("there should be some block");
	sleep(SLEEP_DURATION);
	let session_after = connection.get_session(None).await;
	let block_after = connection.get_best_block().await?.expect("there should be some block");
	assert!(
...
async fn split_test_success_with_all_non_reserved_dead() -> anyhow::Result<()> {
	...
	connection.wait_for_n_sessions(5, BlockStatus::Finalized).await;
