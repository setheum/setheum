...
fn get_initial_reserved_validators(config: &Config) -> Vec<KeyPair> {
	get_validators_raw_keys(config)[..2].iter().map(|k| KeyPair::new(k.clone())).collect()
}
...
fn get_new_reserved_validators(config: &Config) -> Vec<KeyPair> {
	get_validators_raw_keys(config)[3..].iter().map(|k| KeyPair::new(k.clone())).collect()
}
...
async fn get_current_and_next_era_reserved_validators<C: ElectionsApi>(
	...
	(HashSet::from_iter(current_reserved), HashSet::from_iter(stored_reserved))
}
...
async fn get_current_and_next_era_non_reserved_validators<C: ElectionsApi>(
	...
	let current_non_reserved = connection.get_current_era_validators(None).await.non_reserved;
	(HashSet::from_iter(current_non_reserved), HashSet::from_iter(stored_non_reserved))
}
...
pub async fn era_validators() -> anyhow::Result<()> {
	...
	let initial_non_reserved_validators = account_ids_from_keys(&initial_non_reserved_validators_keys);
	...
	root_connection
		.change_validators(
			...
			Some(CommitteeSeats { reserved_seats: 2, non_reserved_seats: 2, non_reserved_finality_seats: 2 }),
			TxStatus::InBlock,
		)
		.await?;
	root_connection.wait_for_n_eras(1, BlockStatus::Finalized).await;
	...
	root_connection.wait_for_session(1, BlockStatus::Finalized).await;
	let (eras_reserved, stored_reserved) = get_current_and_next_era_reserved_validators(&connection).await;
	let (eras_non_reserved, stored_non_reserved) = get_current_and_next_era_non_reserved_validators(&connection).await;

	assert_eq!(
		stored_reserved, new_reserved_validators,
		"Reserved validators' storage not properly updated after change_validators."
	);
	assert_eq!(eras_reserved, initial_reserved_validators, "Reserved validators set has been updated too early.");
