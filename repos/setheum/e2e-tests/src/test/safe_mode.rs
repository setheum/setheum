...
async fn safe_mode_operations_are_disabled_for_users() -> anyhow::Result<()> {
	...
	should_be_err(connection.enter(TxStatus::InBlock).await, "Enter function should fail for any user");
	should_be_err(connection.extend(TxStatus::InBlock).await, "Extend function should fail for any user");
...
async fn safe_mode_is_configured_correctly() -> anyhow::Result<()> {
	...
	let should_be_fails = vec![
		(signed_connection.add_proxy(test_account(), ProxyType::Any, 0, TxStatus::InBlock), "Proxy call should fail"),
		(signed_connection.transfer_keep_alive(test_account(), 100, TxStatus::InBlock), "Balance call should fail"),
		(signed_connection.set_keys(vec![0; 64].into(), TxStatus::InBlock), "Session call should fail"),
		(signed_connection.validate(10, TxStatus::InBlock), "Staking call should fail"),
		(signed_connection.vest(TxStatus::InBlock), "Vesting call should fail"),
	];

	let should_be_successes = vec![
		(root.force_new_era(TxStatus::InBlock), "Root call should succeed"), // sudo with a call that normally would be filtered should still pass through
	];
