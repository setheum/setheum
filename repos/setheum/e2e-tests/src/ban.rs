...
pub async fn check_ban_config<C: CommitteeManagementApi>(
	...
	assert_eq!(ban_config.minimal_expected_performance.0, expected_minimal_expected_performance.deconstruct());
	assert_eq!(ban_config.underperformed_session_count_threshold, expected_session_count_threshold);
	assert_eq!(ban_config.clean_session_counter_delay, expected_clean_session_counter_delay);
...
pub async fn check_underperformed_validator_session_count<C: CommitteeManagementApi>(
	...
	assert_eq!(underperformed_validator_session_count, expected_session_count);
...
pub async fn check_underperformed_validator_reason<C: CommitteeManagementApi>(
	...
	let validator_ban_info = connection.get_ban_info_for_validator(validator.clone(), None).await;
...
pub async fn check_ban_event<C: SetheumWaiting>(
	connection: &C,
...
pub async fn check_underperformed_count_for_sessions<C: AsConnection + Sync>(
	...
	let validators: Vec<_> = reserved_validators.iter().chain(non_reserved_validators.iter()).collect();
	...
	for session in start_session..end_session {
		...
		let previous_session_end_block_hash = connection.get_block_hash(previous_session_end_block).await?;
		...
		for &val in validators.iter() {
			info!("Checking session count | session {} | validator {}", session, val);
			...
			let previous_session_underperformed_count = connection
				.get_underperformed_validator_session_count(val.clone(), previous_session_end_block_hash)
				.await
				.unwrap_or_default();

			let underperformed_diff = session_underperformed_count.abs_diff(previous_session_underperformed_count);
