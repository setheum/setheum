...
use setheum_client::{
	pallets::{
		committee_management::CommitteeManagementApi, elections::ElectionsApi, session::SessionApi, staking::StakingApi,
	},
...
pub async fn compute_session_committee<C: AsConnection + Sync>(
	...
	let validators = connection.get_current_era_validators(first_block_in_era).await;
	...
	Ok(committee.into_iter().partition(|id| validators.reserved.contains(id)))
}
...
pub async fn get_and_test_members_for_session<C: AsConnection + Sync>(
	...
	let reserved_members_bench = get_bench_members(&era_validators.reserved, &reserved_members_for_session);
	let non_reserved_members_bench = get_bench_members(&era_validators.non_reserved, &non_reserved_members_for_session);
	let members_bench = empty().chain(reserved_members_bench).chain(non_reserved_members_bench).collect();

	let members_active: Vec<_> =
		empty().chain(reserved_members_for_session).chain(non_reserved_members_for_session).collect();
	...
	debug!("members for session - computed {:?} ; retrieved {:?}", members_active, network_members);
...
fn get_bench_members(all_members: &[AccountId], members_active: &[AccountId]) -> Vec<AccountId> {
	all_members.iter().filter(|account_id| !members_active.contains(account_id)).cloned().collect()
}
