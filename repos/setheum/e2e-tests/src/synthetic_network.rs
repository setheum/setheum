...
use synthetic_link::{
	IpPattern, NonEmptyString, PortRange, Protocol, QualityOfService, SyntheticFlow, SyntheticNetwork,
...
impl SyntheticNetworkConfigurator {
	...
	fn set_bit_rate(&mut self, bits_per_second: u64, node: Ipv4Addr, port_range: PortRange) -> &mut Self {
		...
		let flow = self.config.flows.iter_mut().find(|flow| flow.label.as_ref() == &label);
		let flow = if let Some(flow) = flow {
			flow
		} else {
			let flow = SyntheticFlow::new(NonEmptyString::new(label).expect("provided non-empty label"));
			self.config.flows.push(flow);
			self.config.flows.last_mut().expect("should be able to get last element of a non-empty Vec")
		};
	...
	pub fn connect_node_to_port_range(&mut self, nodes: impl IntoIterator<Item = (Ipv4Addr, PortRange)>) -> &mut Self {
		for (node, port_range) in nodes {
...
pub async fn set_out_latency(milliseconds: Milliseconds, synthetic_url: String) -> anyhow::Result<()> {
	info!("setting out-latency of node {} to {}ms", synthetic_url, milliseconds);
	...
	client.commit_config(&config.into()).await.context("unable to commit network configuration")
}
...
impl ConnectivityConfiguration {
	pub fn new() -> Self {
		Self { to_connect: HashMap::new(), to_disconnect: HashMap::new() }
	}
	...
	pub fn connect_with_ports(&mut self, to_connect: impl IntoIterator<Item = (Ipv4Addr, PortRange)>) -> &mut Self {
...
impl NodesConnectivityConfiguration {
		...
		for (node, config) in config.0 {
			match self.0.entry(node) {
				Entry::Occupied(mut entry) => {
					...
					entry.connect_with_ports(config.to_connect).disconnect_with_ports(config.to_disconnect)
				},
	...
	pub fn set_config(&mut self, node: &NodeConfig, configuration: ConnectivityConfiguration) {
		self.0.insert(node.synthetic_network_url().to_string(), configuration);
	}
}

async fn wait_for_further_finalized_blocks(connection: &SignedConnection, blocks_to_wait: u32) -> anyhow::Result<()> {
	...
	info!("Current finalized block #{}, waiting for block #{}", finalized_number, block_number_to_wait);
	connection.wait_for_block(|n| n > block_number_to_wait, BlockStatus::Finalized).await;
	Ok(())
}
...
pub async fn await_new_blocks<'a>(
	...
	for (node_name, connection) in nodes.iter() {
		...
		connection.wait_for_block(|block| block >= wait_for_block, BlockStatus::Best).await;
	}
	Ok(best_seen_block)
}
...
pub async fn await_finalized_blocks<'a>(
	...
	for (node_name, connection) in nodes.iter() {
		...
		let finalized_number = connection
			.get_block_number(finalized)
			.await?
			.ok_or(anyhow::anyhow!("Failed to retrieve block number for hash {finalized:?} at node {node_name}"))?;
	...
	for (node_name, connection) in nodes.iter() {
		...
		connection.wait_for_block(|n| n > wait_block, BlockStatus::Finalized).await;
	}
	Ok(())
}
...
pub async fn test_latency_template_test(
	...
	join_all(connections.iter().map(|connection| connection.wait_for_session(1, BlockStatus::Finalized))).await;
	...
	try_join_all(
		connections
			.iter()
			.map(|connection| wait_for_further_finalized_blocks(connection, blocks_to_wait_in_first_session)),
	)
	.await?;
	Ok(())
}
