...
# This script runs locally setheum-node consensus, ie set of setheum-node processes that are either
...
#
# Before run, a chainspec is generated that is an initial testing Setheum chain configuration,
...
# Also, each SetheumNode chain has sudo account. Here we assume it is //Alice.
#
...
setheum_node_root_dir=$(realpath "${script_dir}/..")
pushd "${setheum_node_root_dir}" > /dev/null
...
Usage:
   $0
    ...
    [--dont-build]
       set if you do not want to build testing setheum-node binary
    [--dont-delete-db]
...
function get_ss58_address_from_seed() {
  local seed="$1"
  local setheum_node_path="$2"

  echo $(${setheum_node_path} key inspect --output-type json ${seed} | jq -r '.ss58Address')
}
...
function run_node() {
  ...
  ./target/release/setheum-node "${node_args[@]}"  2> "${BASE_PATH}/${node_name}.log" > /dev/null &
}
...
if ! killall -9 setheum-node 2> /dev/null; then
