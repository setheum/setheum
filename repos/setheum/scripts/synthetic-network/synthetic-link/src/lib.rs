...
const DEFAULT_SYNTHETIC_NETWORK: SyntheticNetwork =
	SyntheticNetwork { default_link: DEFAULT_SYNTHETIC_LINK, flows: Vec::new() };

const DEFAULT_SYNTHETIC_LINK: SyntheticLink = SyntheticLink { ingress: DEFAULT_QOS, egress: DEFAULT_QOS };
...
const DEFAULT_FLOW: Flow = Flow { ip: IpPattern::All, protocol: Protocol::All, port_range: PortRange::all() };
...
impl SyntheticFlow {
	pub fn new(label: NonEmptyString) -> Self {
		Self { label, flow: DEFAULT_FLOW, link: DEFAULT_SYNTHETIC_LINK }
	}
}
...
impl From<PortRange> for PortRangeSerde {
	fn from(value: PortRange) -> Self {
		PortRangeSerde { port_min: *value.0.start(), port_max: *value.0.end() }
	}
}
...
impl SyntheticNetworkClient {
	pub fn new(url: String) -> Self {
		SyntheticNetworkClient { client: Client::new(), url }
	}
