...
# setheum_node cli options to env variables
...
SETHEUMBFT_NETWORK_BIT_RATE=${SETHEUMBFT_NETWORK_BIT_RATE:-}
...
if [[ "true" == "$PURGE_BEFORE_START" ]]; then
  ...
  setheum-node purge-chain --base-path "${BASE_PATH}" --chain "${CHAIN}" -y
fi
...
echo "${CUSTOM_ARGS}" | xargs setheum-node "${ARGS[@]}"
