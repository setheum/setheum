...
impl SudoCall for RootConnection {
    async fn sudo_unchecked(&self, call: Call, status: TxStatus) -> anyhow::Result<TxInfo> {
        info!(target: "setheum-client", "sending call as sudo_unchecked {:?}", call);
    ...
    async fn sudo(&self, call: Call, status: TxStatus) -> anyhow::Result<TxInfo> {
        info!(target: "setheum-client", "sending call as sudo {:?}", call);
...
impl<C: AsConnection + Sync> ConnectionApi for C {
    ...
    >(
        ...
        info!(target: "setheum-client", "accessing storage at {}::{} at block {:?}", addrs.pallet_name(), addrs.entry_name(), at);
    ...
    async fn rpc_call<R: Decode>(&self, func_name: String, params: RpcParams) -> anyhow::Result<R> {
        info!(target: "setheum-client", "submitting rpc call `{}`, with params {:?}", func_name, params.clone().build());
...
impl<S: AsSigned + Sync> SignedConnectionApi for S {
    ...
    async fn send_tx_with_params<Call: TxPayload + Send + Sync>(
        ...
        if let Some(details) = tx.validation_details() {
            info!(
                target:"setheum-client", "Sending extrinsic {}.{} with params: {:?}",
                details.pallet_name,
                details.call_name,
                params,
            );
        ...
        info!(target: "setheum-client", "tx with hash {:?} included in block {:?}", info.tx_hash, info.block_hash);
